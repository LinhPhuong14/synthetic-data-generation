...
def test_corpus_en_tag_matches_the_english_corpus_on_disk():
    """`corpus_en` means "corpus/en/ covers this document's content profile".

    Measured, not asserted. `content.build` merges `{**document, **content}`, so
    a content value carrying `lang: en` sends every corpus lookup to
    `rulebase/corpus/en/` -- and a profile with no English files there does not
    fall back, it raises `FileNotFoundError` in the middle of a shard. Measured
    on the run that found this: 34 of 47 documents would have died that way, on
    `shops_utility_water.txt` first.

    So the tag is a claim about files, and this is the check. Adding
    `items_eatery.txt` and `shops_eatery.txt` to `corpus/en/` is what opens the
    four eatery documents to English -- by tagging them here and watching this
    test go green, not by editing a list in `content.yaml`.
    """
    import sys
    ...
    english = root / "rulebase" / "corpus" / "en"
    covered = {path.stem.split("_", 1)[1] for path in english.glob("items_*.txt")}
    covered &= {path.stem.split("_", 1)[1] for path in english.glob("shops_*.txt")}

    wrong = []
    for option in load_rules()["document"]:
        profile = (option.params or {}).get("profile")
        tagged = "corpus_en" in option.tags
        if tagged and profile not in covered:
            wrong.append(f"{option.id} is tagged corpus_en but corpus/en/ has no "
                         f"items_{profile}.txt/shops_{profile}.txt")
        if not tagged and profile in covered:
            wrong.append(f"{option.id} (profile {profile}) could draw English and "
                         "is not tagged corpus_en")
    assert not wrong, "\n".join(wrong)
