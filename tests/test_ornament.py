...
def test_something_actually_strikes_the_mark_onto_the_page():
    ...
    unit test in this file passed against a renderer that never struck a mark,
    because they strike it themselves; and the checks that could have noticed
    -- preflight's two-way asset audit, `rules_report --check` -- were all
    looking at whether the rules and the FILES agree, never at whether a page
    got one.

    WHERE the striking happens moved: it used to be `ornament.stamp()` on the
    finished screenshot in `generators/html/render.py`, and it is now done in
    the MARKUP before the screenshot, so the mark's own label box is measured
    off the DOM like every other run (see `render.py`'s own comment beside the
    ageing chain, and `sheets/base.py::seal_mark`). This test follows the
    mechanism rather than the old call, because its point was never that one
    function is called -- it is that some page-anchored mark reaches the paper.

    Reading the source is crude. It is also the cheapest thing that fails when
    somebody removes the hook, and the dependency-free CI job cannot start
    Chromium to check the pixels instead.
    """
    sheets = REPO_ROOT / "generators" / "html" / "sheets"
    dispatch = (sheets / "__init__.py").read_text(encoding="utf-8")
    base_source = (sheets / "base.py").read_text(encoding="utf-8")
    assert "def render_ornament_marks(" in base_source, (
        "sheets/base.py no longer draws the sampled marks at all")
    assert "render_ornament_marks(recipe, receipt)" in dispatch, (
        "sheets/__init__.py no longer strikes page-anchored marks; the "
        "attribute would be recorded on every page and printed on none")
    assert "_page_ornaments(markup, recipe, receipt)" in dispatch, (
        "sheets/__init__.py::build no longer runs the page-mark hook, so a "
        "family that does not draw its own overlay gets no mark")
