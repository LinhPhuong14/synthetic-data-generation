...
def test_every_document_has_at_least_one_blank_and_no_stray_names(real_rules):
    """The contract: the two sides name the same documents, and none is empty.

    This used to pin two counts (`hospital_bill == 1`, `pub_eatery == 3`) and
    its own docstring admitted "this count moves with it" -- which is another
    way of saying the numbers were a snapshot, not a rule. They moved: the
    `_spli`/`_part`/`_titl` variants took `hospital_bill` to four, and three
    hand-written eatery phôi took `pub_eatery` to six. A test that has to be
    edited every time a phôi is added is not protecting anything; it just
    fails last.

    What IS a rule, and what actually breaks a run when it is false: a
    document the rules can draw with NO blank to draw it from. `sample_recipe`
    picks the document first, then looks for a layout, and an empty list there
    is a walk that dead-ends on every seed.
    """
    ...
    assert empty == [], f"chứng từ không có phôi nào để vẽ: {empty}"
