...
@pytest.mark.parametrize("kind,label", [
    ...
    # là chữ chạy, xét theo NGHĨA chứ không theo chỗ nó đứng. Tiêu đề bảng --
    # thứ thật sự là một đầu mục -- có `kind` riêng là `caption.table`.
    ("subtitle", "Text"),
    ("caption.table", "Section-header"),
    # Quốc hiệu và tiêu ngữ: mọi công văn đều in đúng hai dòng ấy, nên chúng
    # không nói tờ này là giấy gì. Gọi là `Title` thì một trang có HAI `Title`
    # và không cái nào là tên chứng từ.
    ("masthead", "Text"),
    ("store.name", "Page-header"),
    # A `.label` half follows its own family without being listed for it.
    ("store.address.label", "Page-header"),
    # Khối tổng KHÔNG phải bảng: cùng một kho in nó bằng `<tr><td>` ở chỗ
    # này và bằng `<div class="trow">` ở chỗ kia, nên `kind` không quyết được.
    # Từ nào thật sự nằm trong `<td>` thì `regions_from_words` nâng lên
    # `Table`; ở đây, ngoài mọi ngữ cảnh DOM, nó là một khối chữ.
    ("total.grand.label", "Text"),
    ("menu.unit_price", "Table"),
    # `Page-footer` dành cho thứ THẬT SỰ ở chân trang -- số trang. Dòng ghi
    # chú cuối tờ ("Công ty chỉ trả sau khi đối chiếu chữ ký...") là một câu
    # của chứng từ, tình cờ in ở dưới.
    ("footer", "Text"),
    ("footer.page", "Page-footer"),
    # Khung dán ảnh chỉ có chữ "ẢNH" và "4 x 6" in trong một đường viền --
    # không có tấm ảnh nào. Chữ chìm cũng vậy: mờ và xoay, nhưng đọc được.
...
def test_a_word_inside_a_measured_cell_is_table_whatever_its_kind_says():
    """`Table` là thứ một phần tử LÀ, không phải thứ chữ của nó NGHĨA LÀ.

    Cùng một kho in khối tổng bằng hai cách -- `<tr><td>` trong bảng hàng
    (`sheets/base.py::_item_row`, `sheets/medical.py`) và `<div class="trow">`
    đứng riêng (khối `.totals`, `synthgen/markup.py::_totals`). Cùng `kind`,
    khác DOM, nên bảng tra theo `kind` phải nói dối một trong hai. Luật là:
    ô đo được thắng."""
    inside = a_box(kind="total.grand", text="232.000",
                   quad=[[10, 10], [120, 10], [120, 30], [10, 30]])
    outside = a_box(kind="total.grand", text="232.000",
                    quad=[[10, 900], [120, 900], [120, 920], [10, 920]])
    words = R.words_from_boxes([inside, outside])
    cell = {"quad": [[0, 0], [400, 0], [400, 60], [0, 60]], "text": "232.000",
            "row": 1, "col": 0}
    R.regions_from_words(words, cells=[cell], page_size=(600, 1000))
    assert words[0]["layout_class"] == "Table"   # nằm trong ô đo được
    assert words[1]["layout_class"] == "Text"    # cùng kind, ngoài mọi ô
...
def test_a_block_keeps_the_field_it_is_as_well_as_the_class_it_is_in():
    ...
    # `Text`, không phải `Table`: `blocks_from_boxes` chỉ thấy `kind`, và một
    # dòng tổng đứng ngoài mọi ô bảng là một khối chữ. Nó thành `Table` khi và
    # chỉ khi nó nằm trong một `<td>` đo được -- xem
    # `test_a_word_inside_a_measured_cell_is_table_whatever_its_kind_says`.
    assert block["label"] == "Text"           # what the converter calls it
...
@pytest.mark.parametrize("break_it,expected", [
    ...
    (lambda i: i.update(source_files=["other.jpg"]), "source_files must start with"),
    # A document may span sheets, so the count is checked against `pages[]`
    # rather than pinned at one -- see `record.build(sheets=...)`.
    (lambda i: i.update(source_files=["html_000.jpg", "html_000_p2.jpg"]),
     "one file per printed side"),
    (lambda i: i.update(pages=[]), "at least one page"),
    (lambda i: i["pages"].append({"page_number": 3, "width": 9, "height": 9}),
     "numbered from 1 in print order"),
