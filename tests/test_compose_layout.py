...
def test_the_regions_are_those_of_axis_one():
    """R-2. Read from the converter's own label set, so a composition cannot
    name a region no record will ever carry.

    Cái tên cũ chốt con số mười tám, và dòng `assert len(got) == 18` chỉ nói
    lại điều phép so tập hợp ngay dưới đã nói -- nhưng nó cũ đi ngay khi từ
    vựng lớn lên có lý do: `Watermark` và `Stamp` thêm vào để tách mực phủ ra
    khỏi nội dung chính. Một con số viết cứng bên cạnh một phép so phái sinh
    không bắt thêm được lỗi nào; nó chỉ bắt chính người sửa từ vựng."""
    from pipeline.record import DOCSYNTH_LABELS

    got = compose.regions()
    assert len(got) == len(DOCSYNTH_LABELS) - 1, "trừ đúng WHOLE_PAGE"
