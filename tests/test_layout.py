...
def test_a_thermal_layout_is_on_a_roll_and_an_invoice_is_on_a_sheet():
    ...
    # The sheet a layout gets is the sheet it DECLARES, not a4 for everyone.
    # This used to assert `grid.sheet == "a4"` for every cut sheet, and
    # `form_cash_voucher` (a5) has been failing it since the day a smaller
    # sheet shipped -- an A5 voucher is not a bug, and a test that calls it one
    # hides the property actually worth holding: whatever paper the layout
    # names, that is the paper the grid reports and the ratio matches it.
    from rulebase.style import SHEETS

    for layout_id in LAYOUTS:
        declared = str(rulebase.load_layout(layout_id).get("sheet") or "")
        ...
        if layout_id in rolls:
            ...
            assert grid.sheet == declared, layout_id
            assert declared in SHEETS, f"{layout_id}: giấy lạ {declared!r}"
            assert ratio == pytest.approx(SHEETS[declared]), layout_id
