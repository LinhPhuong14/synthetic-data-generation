...
def test_load_rules_sees_a_file_that_changed_under_it(tmp_path):
    """`load_rules` nhớ kết quả, và cái nhớ ấy phải hết hiệu lực khi file đổi.

    Vì sao có bộ nhớ đệm: `_load_rules` phân tích lại mọi file YAML mỗi lần
    gọi, và `rulebase.make()` gọi nó một lần cho mỗi trang -- đo được 2.0 giây
    một trang, gần như toàn bộ là đọc lại đúng những file vừa đọc. Preflight
    dựng 12 trang cho MỖI phôi, nên nó trả giá ấy hơn một nghìn lần.

    Vì sao phải có test này: khoá chỉ theo đường dẫn thì một lượt chạy vừa
    ...
    after = load_rules(root)["color"]
    assert len(after) == before + 1
    assert "mực_tím_thêm_sau" in {option.id for option in after}
...
def test_two_rules_roots_do_not_share_one_cached_answer(tmp_path):
    ...
    ids_one = {option.id for option in load_rules(one)["color"]}
    ids_two = {option.id for option in load_rules(two)["color"]}
    assert "chỉ_có_ở_cây_hai" in ids_two
    assert "chỉ_có_ở_cây_hai" not in ids_one
