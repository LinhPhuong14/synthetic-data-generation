...
def _checklist(receipt, spec: dict, rng: random.Random) -> str:
    ...
    rows = []
    for line in lines:
        ticked = rng.random() < rate
        # Ô TÍCH CÓ HỘP, và trạng thái của nó nằm trong nhãn. Bản trước cố ý
        # không gắn `data-kind`, với lý do "trạng thái của nó là một lần tung
        # đồng xu lúc vẽ, không phải điều tờ giấy NÓI". Lý do ấy đúng một nửa:
        # đồng xu này tung bằng `rng` của `build()`, gieo từ `recipe.seed`, nên
        # nó dựng lại được y như mọi thứ khác trên trang -- chính docstring của
        # `_roster_table` viết thế. Và docstring của module này đã tự đặt ra
        # luật: "vẫn là mực thật người đọc phải đọc, nên nó mang `span()` như
        # mọi thứ khác".
        #
...
def _total_entries(parse: dict) -> list[dict]:
    """`parse["total"]` ở hình dạng dòng tổng cần. Xem `modern._total_entries`."""
...
def _item_table(spec: dict, receipt, parse: dict, rows,
                totals: list[dict] | None = None) -> str:
    ...
    # Dòng tổng là DÒNG CỦA CHÍNH BẢNG NÀY, không phải một khối vẽ bên dưới.
    # Nhãn `colspan` hết phần trái, số nằm đúng dưới cột cuối -- đó là thứ
    # `colspan` sinh ra để làm, và là khác biệt giữa "trông giống bảng" với
    # "là bảng". Vẽ rời thì `pipeline/record.py` buộc phải gán nhãn chữ thường
    # cho một khối vốn LÀ bảng, và mọi mép cột phải tự canh cho khớp bằng tay.
    span_left = max(len(columns) - 1, 1)
    for entry in totals or []:
        grand = bool(entry.get("grand"))
        kind = "total.grand" if grand else "total.line"
        table_rows.append(Row([
            Cell(span(f"{kind}.label", str(entry.get("label", ""))), html=True,
                 colspan=span_left, align="right",
                 cls="tlabel grand" if grand else "tlabel", bold=grand or None),
            Cell(span(kind, str(entry.get("value", ""))), html=True,
                 align="right", cls="grand" if grand else "", bold=grand or None),
        ]))
...
def _section_html(name: str, receipt, spec: dict, parse: dict, sections: list,
    ...
    if name == "parties":
        ...
        return block
    if name == "table":
        # Bảng chấm công (`grid`) không có cột tiền nào để cộng, nên nó vẫn
        # dùng `totals_block` như cũ; chỉ bảng khoản mục mới nhận dòng tổng.
        table = (_roster_table(spec, receipt, parse, rows, rng) if spec.get("grid")
                else _item_table(spec, receipt, parse, rows,
                                 _total_entries(parse) if "totals" in sections else None))
        sectioned = spec.get("sectioned")
        if sectioned:
            titles = sectioned.get("titles") or ["Thông tin chung", "Số liệu"]
    ...
    if name == "totals":
        # Đã in ở trong bảng rồi (xem nhánh `table` trên). Bảng chấm công thì
        # chưa, nên nó vẫn vẽ khối riêng.
        if "table" in sections and not spec.get("grid"):
            return ""
...
def build(recipe, receipt, spec: dict, parse: dict) -> str:
    ...
    #
    # A công văn or a biên bản is a page of running text with a signature block
    # under it, and when the text runs long the sheet has always grown instead
    # of turning over. See `rulebase/layout.py::page_order`.
    ...
    if sheet_pages:
        ...
        bounds = base.split_items(len(receipt.items), len(table_on) or 1,
                                  first=(spec.get("table") or {}).get("rows_first"))
        # Which receipt each sheet's TABLE draws from -- a slice of the items,
        # so a roster running onto a second sheet carries the rest of the names
        # rather than all of them again. Every other block reads the whole
        # receipt: a signature block is not half a signature block.
        sliced = {index: base.page_receipt(receipt, *(bounds[slot]
                                                      if slot < len(bounds) else (0, 0)))
                  for slot, index in enumerate(table_on)}
        if len(table_on) > 1 and not any(low < high for low, high in bounds[1:]):
            # Nothing spilled, so there is no second sheet -- see `modern.build`.
            merged, seen_table = [], False
            for names in sheet_pages:
                for name in names:
        ...
        for index, names in enumerate(sheet_pages):
            pieces = [_section_html(
                name,
                sliced.get(index, receipt) if name == base.TABLE_SECTION else receipt,
                spec, parse, sections, rng, rows,
                stamp=stamp_slot, totals_stamp=totals_stamp) for name in names]
