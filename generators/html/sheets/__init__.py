...
# Where `base.document()` puts `overlay=`: the first thing inside the FIRST
# sheet. Matched by pattern rather than by a literal since `document()` learned
# to write several sheets -- the opening tag now carries `class`, `id` and
# `data-page`, and a literal that no longer matches would have gone on
# returning the markup unchanged, which looks exactly like a page whose rules
# drew `no_ornament`.
_SHEET_OPEN_RE = re.compile(r'<div class="sheet" id="sheet"[^>]*>')
...
def _page_ornaments(markup: str, recipe, receipt) -> str:
    ...
    match = _SHEET_OPEN_RE.search(markup) if overlay else None
    if match is None:
        return markup
    # A family that ALSO passes `document(overlay=)` gets the mark twice --
    # two identical `<img>` and two identical label boxes on top of each
    # other. `lodging.py` did exactly that until it was measured on
    # `hotel_stay_156`, so this no longer trusts "no family passes one":
    # if the fragment is already inside the sheet, leave it alone.
    if overlay and overlay in markup[match.end():match.end() + len(overlay) + 64]:
        return markup
    return markup[:match.end()] + overlay + markup[match.end():]
