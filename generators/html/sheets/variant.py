...
# `<th>` cells of the item table's header band -- the span a reversed design
# repaints. Bounded by `</thead>` on purpose: `components/table.py` allows a
# nested `TableSpec` inside a body cell, and those `<th>` are a different
# table's heads that this design's `table.items th` rule never touched.
_ITEMS_TABLE = re.compile(r'<table[^>]*class="[^"]*\bitems\b[^"]*"[^>]*>.*?</thead>',
                          re.S)


def mark_reversed(markup: str) -> str:
    """`data-ink="reversed"` on the heads a reversed design paints dark.

    Axis 3 (`pipeline/record.py::INK_VALUES`) has six values and this is the
    only one nothing else can work out. `kind` cannot: a column heading is
    `colhdr` whether it is black on white or white on navy. The recipe's tags
    cannot either: reversing is a property of ONE BAND of one page, not of the
    paper or the print head. And the family that emitted the `<th>` cannot,
    because the design's stylesheet is glued on here, after it has finished.

    So it is done here, where the two facts finally meet -- the markup, and
    the dressing that is about to repaint it.

    Written onto the `<th>` rather than onto every run inside it: `page.py::
    CELL_RECTS_JS` reads `span.closest('[data-ink]')`, so one attribute on the
    cell covers every word in it, and a heading that wraps to two lines does
    not need two.
    """
    def stamp(match: "re.Match[str]") -> str:
        return match.group(0).replace("<th ", '<th data-ink="reversed" ')

    return _ITEMS_TABLE.sub(stamp, markup)
...
def apply(markup: str, recipe) -> str:
    ...
    option = chosen(recipe)
    css = css_of(recipe)
    # Before the early return: a design could in principle reverse the heads
    # through a rule this run has no CSS for, and more importantly the flag is
    # about the DOM rather than about the stylesheet.
    if (getattr(option, "params", None) or {}).get("reversed_header"):
        markup = mark_reversed(markup)
    if not css.strip():
        return markup
    bad = forbidden(css)
    if bad:
