...
def _section_html(name: str, receipt, spec: dict, parse: dict, sections: list,
    ...
    if name == "table":
        # `table.caption` -- một dòng nói bảng này LÀ BẢNG GÌ.
        #
        # Mọi bảng phức tạp ngoài đời đều có: "CHI TIẾT HÀNG HOÁ, DỊCH VỤ",
        # "II. Phần chi phí khám bệnh, chữa bệnh", "BẢNG THỐNG KÊ CỐT THÉP".
        # Họ `medical` in nó từ lâu; họ này thì bảng đứng TRẦN, không câu nào
        # nói nó là bảng gì -- và một bảng mười ba cột không có tên là bảng
        # người đọc phải đoán.
        #
        # `span("note", ...)`: nó là chữ in trên giấy nên phải có hộp nhãn như
        # mọi chữ khác. Không phải `data-kind` mới -- `note` đã là loại "chữ
        # của tờ giấy, không phải giá trị của trường", đúng chỗ một tiêu đề
        # bảng thuộc về.
        caption = str((spec.get("table") or {}).get("caption") or "").strip()
        if caption:
            return f'<div class="tcap">{span("note", caption)}</div>{table}'
        return table
    if name == "totals":
        # Đã in rồi, ở trong chính bảng hàng -- xem `build()`. Trả rỗng chứ
        # không bỏ tên khỏi `sections:`: `sections` còn quyết thứ tự khối và
        # những chỗ khác đọc nó, nên rút một tên ra là đổi một thứ không liên
        # quan tới chỗ dòng tổng được vẽ.
        if not (spec.get("table") or {}).get("component") and "table" in sections:
            return ""
...
def _total_entries(parse: dict) -> list[dict]:
    """`parse["total"]` ở hình dạng `items_table(totals=...)` nhận.

    Cùng bốn dòng `statutory.py` viết, và để ở đây vì `form.py` cần y hệt --
    hai bản của cùng một phép đổi hình dạng là hai bản sẽ lệch nhau ở lần sửa
    đầu tiên. Dòng CUỐI là dòng tổng cộng: đó là quy ước `parse["total"]` đã
    có, không phải một phép đoán."""
...
def build(recipe, receipt, spec: dict, parse: dict) -> str:
    ...
    if table_settings.get("component"):
        ...
        # Totals go IN the table, as merged rows, the way `statutory.py` and
        # `till.py` have always done it -- `items_table` has taken them since
        # it was written and this caller simply never passed them. Drawn
        # underneath instead, as `totals_block` does, they are a `<div
        # class="trow">` grid: it looks like a table to a reader and is not one
        # to the DOM, so `pipeline/record.py` has to label a block that IS a
        # table as running text. Every column edge also has to be kept lined up
        # with the table above by hand, which is the job `colspan` exists for.
        table = base.items_table(
            spec, receipt, parse, rows,
            cls="items grid" if grid else "items",
            totals=_total_entries(parse) if "totals" in sections else None)

    # ---- several printed sides, when the layout says which block starts each
    #
    # Read BEFORE the sidebar branch, and it wins: a rail down the side and a
    # run across two sheets are two different answers to "what shape is this
    # page", not two halves of one. A layout asking for both gets the sheets,
    # because that is the one of the two that decides how much paper is used.
    sheet_pages = base.page_order(spec, sections)
    if sheet_pages:
        table_on = base.table_pages(sheet_pages)
        bounds = base.split_items(len(receipt.items), len(table_on) or 1,
                                  first=table_settings.get("rows_first"))
        # One table per sheet it runs onto, each holding its own slice of the
        # items and each repeating the column header -- `rows` is the SAME
        # counter throughout, so the line numbers carry on across the fold
        # instead of starting again at 1 on the second sheet.
        # Tờ nào mang dòng tổng, quyết TRƯỚC khi dựng bảng. Thường là tờ cuối;
        # nhưng khi không có gì tràn sang tờ sau thì mấy tờ ấy gộp lại còn một
        # và bản được dùng là `tables[0]` -- nên "tờ cuối" ở đây là tờ cuối của
        # bản ĐÃ GỘP, không phải của danh sách dự tính. Tính sai chỗ này thì
        # dòng tổng biến mất khỏi `invoice_long_ledger` và `invoice_multipage`
        # mà không có lỗi nào nổ ra.
        spills = any(low < high for low, high in bounds[1:])
        with_totals = (0 if len(table_on) > 1 and not spills
                       else len(table_on) - 1)
        tables: dict[int, str] = {}
        for slot, index in enumerate(table_on):
            low, high = bounds[slot] if slot < len(bounds) else (0, 0)
            slice_receipt = base.page_receipt(receipt, low, high)
            if table_settings.get("component"):
                tables[index] = _grid_items_table(spec, slice_receipt, parse, rows)
            else:
                # Dòng tổng chỉ đi với tờ CUỐI, và chỉ khi bảng thật sự chạy
                # sang tờ ấy: cộng tiền ở cuối tờ một rồi lại cộng lần nữa ở
                # cuối tờ hai là in hai lần một con số. Bỏ sót nhánh này là
                # đúng cách dòng tổng biến mất khỏi `invoice_long_ledger` và
                # `invoice_multipage` -- `tests/test_sheets.py` bắt được, vì
                # nó so từng giá trị trong nhãn với từng đoạn chữ in ra.
                tables[index] = base.items_table(
                    spec, slice_receipt, parse, rows,
                    cls="items grid" if grid else "items",
                    totals=(_total_entries(parse)
                            if slot == with_totals and "totals" in sections
                            else None))
        # A second sheet is a fact about the CONTENT, not only about the layout:
        # `page_order` says what would go on it, and how many items were drawn
        # says whether there is one. Fourteen lines fill the first sheet and
        # spill; four do not, and printing their totals alone on a second sheet
        # is a document nobody has ever been handed. So when every continuation
        # slice came back empty, the groups are concatenated and this draws the
        # single sheet it always drew -- same blocks, same order, one page.
        # Only when the TABLE is what runs across the sheets: a layout that
        # splits blocks without repeating `table` has no spill to measure,
        # and collapsing on that would make `page_order` a no-op on every
        # text-only document that declares one.
        if len(table_on) > 1 and not spills:
            sheet_pages = [[name for names in sheet_pages for name in names]]
            # `table` keeps the whole item list in that case: `tables[0]` holds
            # only the first slice, which is every item precisely because
            # nothing spilled -- but the second group's repeated `table` name
            # would otherwise draw it a second time.
            seen_table = False
            merged = []
            for name in sheet_pages[0]:
                if name == base.TABLE_SECTION:
                    if seen_table:
                        continue
                    seen_table = True
                merged.append(name)
            sheet_pages = [merged]
        # The same fallback the single-sheet path below has, and it has to be
        # here too: a layout that names no `doctitle` still HAS a title, and
        # `ground_truth()` claims it either way. `invoice_multipage` is exactly
        # such a layout, and without this its two sheets printed a document
        # whose own name appeared nowhere -- caught by
        # `test_what_the_label_says_the_page_prints`, which is what that test
        # is for. It goes on the FIRST sheet, under the shop's name, because
        # that is where the reference sheet puts it.
        if "doctitle" not in sections and sheet_pages:
            first = list(sheet_pages[0])
            first.insert(min(1, len(first)), "doctitle")
            sheet_pages = [first] + [list(page) for page in sheet_pages[1:]]

        bodies = []
        for index, names in enumerate(sheet_pages):
            pieces = [_section_html(name, receipt, spec, parse, sections,
                                    tables.get(index, table), header_mode,
                                    stamp=stamp_slot, totals_stamp=totals_stamp)
                      for name in names]
            bodies.append("".join(piece for piece in pieces if piece))
        body = bodies
    elif page.get("style") == "sidebar":
    ...
    # Everything below appends to the LAST sheet and prepends to the first,
    # which is where a continuation notice and a watermark belong on a document
    # of any length. Written against a list so the single-sheet case is a list
    # of one rather than a second branch.
    ...
    if marker:
        ...
        # On every sheet that IS followed by another, and on no other. The
        # string is a "continues overleaf" notice, so a sheet with nothing
        # after it must not carry one -- which `invoice_multipage.yaml` did
        # carry, on its single sheet, for as long as the renderer could not
        # actually turn a page. That was a stand-in; this is the page.
        stub = f'<div class="pagemark">{span("page_no", marker)}</div>'
        for position in range(len(bodies) - 1):
            bodies[position] += stub
    ...
    if page.get("watermark"):
        ...
        if wm_text:
            bodies[0] = f'<div class="wm">{base.esc(wm_text)}</div>' + bodies[0]
...
.tcap{{font-weight:bold;font-size:8.2pt;letter-spacing:.3pt;margin:0 0 2mm;}}
...
"""
    ...
    if narrow:
        return base.document(bodies, css, paper="A5", padding="12mm 11mm 10mm",
                             font=font, size="7.4pt", colour="#1c1c1c",
                             line_height="1.45")
    return base.document(bodies, css, paper="A4", padding="16mm 15mm",
