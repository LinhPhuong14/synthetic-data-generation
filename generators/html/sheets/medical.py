...
def build(recipe, receipt, spec: dict, parse: dict) -> str:
    ...
    def pieces_for(name: str, table_html: str, *, heading: bool = True) -> list[str]:
        """The html of one named block. One dispatch, so the plain top-to-bottom
        flow and the sheet split below both ask for "block X" rather than the
        second one growing its own copy of this ladder.

        `heading`: a table CONTINUED on a later sheet keeps its column header
        (the `<thead>` repeats) but must not print section II's caption again --
        "II. Phần chi phí khám bệnh, chữa bệnh" announces the section once, and
        a second announcement reads as a second section.
        """
        if name in ("letterhead", "strip"):
            return []                 # both are the head, drawn once
        if name == "doctitle":
            return [span("title", parse.get("title", ""), "doc")]
        if name == "parties":
            return [f'<div class="sec">{span("note", SECTION_I)}</div>',
                    _fields(receipt, parse, spec)]
        if name == "table":
            caption = (f'<div class="sec">{span("note", SECTION_II)}'
                       f'<span class="reg"> {span("note", SECTION_II_NOTE)}</span></div>'
                       if heading else "")
            return [caption, table_html]
        if name == "totals":
            return [_settlement(parse)]
        if name == "words":
            return [base.words_block(receipt, parse)]
        if name == "signatures":
            return [base.signature_block(receipt, parse, stamp=stamp_slot)]
        if name == "footer":
    ...
    #
    # A hospital bill is the document this exists for: thirteen columns and as
    # many lines as the stay had, on a sheet whose height was decided before
    # anything was printed. See `rulebase/layout.py::page_order`.
    ...
    if sheet_pages:
        ...
        for slot, index in enumerate(table_on):
            ...
            piece = base.items_table(spec, base.page_receipt(receipt, low, high),
                                     parse, rows)
            # The summing row closes the table, so it belongs to the LAST sheet
            # the table runs onto -- and it sums the WHOLE receipt, because a
            # total split across two sheets is not a total.
            if piece and columns and index == table_on[-1]:
                piece = piece.replace(
                    "</tbody></table>",
                    _sum_row(receipt, spec, columns, rows) + "</tbody></table>")
            tables[index] = piece

        # A second sheet is a fact about the CONTENT as well: a four-line bill
        # printing its settlement block alone overleaf is not a document anybody
        # has been handed. Nothing spilled -> the groups concatenate and this
        # draws the single sheet it always drew.
        # Only when the table is what spans them -- see `modern.build`.
        ...
        for index, names in enumerate(sheet_pages):
            out: list[str] = []
            for name in names:
                out.extend(pieces_for(name, tables.get(index, table),
                                      heading=index == table_on[0]
                                      if name == base.TABLE_SECTION else True))
            # The head -- name, address, the form's own number -- opens the
            # document, not every sheet of it.
            opening = _head(receipt, parse, spec) if index == 0 else ""
            bodies.append(opening + "".join(piece for piece in out if piece))
        body = bodies
    else:
        blocks: list[str] = []
        for name in sections:
            blocks.extend(pieces_for(name, table))
        body = _head(receipt, parse, spec) + "".join(
