...
def build(recipe, receipt, spec: dict, parse: dict) -> str:
    ...
    # `overlay` deliberately unused: `sheets/__init__.py::_inject_page_marks`
    # puts every page-anchored mark inside `#sheet` for EVERY family, from
    # outside. This family used to pass it to `document()` as well, so a
    # `copy_stamp` page carried the same seal twice -- two identical `<img>`
    # and two identical `seal.square` label boxes at IoU 1.0, which
    # `tools/layout_audit.py` reported on `hotel_stay_156` and
    # `resort_stay_179`. One striking, one box.
    stamp_slot, totals_stamp, _overlay_drawn_centrally = base.render_ornament_marks(
        recipe, receipt)
