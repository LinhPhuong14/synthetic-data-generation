...
# being true.
# MỌI thuộc tính sau `data-kind`, không riêng `class`. Bản trước chỉ nhận
# `data-kind` và tuỳ chọn `class`, nên một run mang thêm thuộc tính nào khác là
# KHÔNG KHỚP -- và không khớp thì im lặng: `fill` trả về "đã rót 0 ô, từ chối 0
# ô", đọc y hệt một trang không có ô nào để điền. `synthgen/markup.py` gắn
# `data-role` và `data-key` lên run của nó, nên cả bộ sinh ấy không rót được
# một nét mực nào và không gì báo.
RUN = re.compile(r'<span data-kind="([^"]+)"([^>]*)>([^<>]*)</span>')
...
def _kept(attrs: str) -> str:
    """Những thuộc tính KHÁC `class` của run gốc, giữ nguyên văn.

    `span()` dựng lại thẻ từ đầu, nên thứ gì không chép sang là thứ bị mất.
    `data-role` và `data-key` quyết việc ghép cặp KIE (`pipeline/kie.py` đọc
    vai trò khoá/giá trị từ đó), nên đánh rơi chúng là rót mực xong thì mất
    nhãn -- một cái giá không ai định trả cho việc đổi phông chữ."""
    out = re.sub(r'\s*class="[^"]*"', "", attrs).strip()
