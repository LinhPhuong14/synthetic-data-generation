...
<body><div class="sheet" id="sheet" data-page="1">{"".join(marks)}{"".join(spans)}{tint_layer}</div></body></html>"""
...
def regions_from_rects(rects, scale: float, factor: float) -> list[dict]:
    ...
    for rect in rects:
        ...
        cells.append({
            ...
            "table": int(rect.get("table", 0) or 0),
            "row": rect["row"], "col": rect["col"],
            # `head` đi cùng `row`/`col`: một ô ở `<thead>` là ĐẦU CỘT, và vai
            # ấy chắc hơn mọi phép suy từ `kind`. Bản trước rơi mất nó, nên
            # `run_role` không bao giờ trả `colhdr` -- đo được 0 trên 17 882
            # run của pilot14.
            "head": bool(rect.get("head")),
...
def zones_from_rects(rects, scale: float, factor: float) -> list[dict]:
    """Khối tự khai vùng: `{label, quad}`, theo pixel của chính tấm ảnh.

    Cùng hai phép nhân với `quads_from_rects`, và cũng cố ý KHÔNG dùng lại
    nó: một khối không có `text` riêng -- chữ nằm ở các từ bên trong -- nên
    đi qua hàm ấy là bị bỏ hết vì rỗng chữ."""
    ratio = scale * factor
    out = []
    for rect in rects:
        if rect["w"] <= 0 or rect["h"] <= 0:
        ...
        x1, y1 = x0 + rect["w"] * ratio, y0 + rect["h"] * ratio
        out.append({
            "label": str(rect.get("label", "") or "Text"),
            # XUẤT XỨ: `declared` là model tự khai `data-region`, `tag` là suy
            # từ thẻ ngữ nghĩa. `_unwrap` cần phân biệt -- xem chú thích ở đó.
            "from": str(rect.get("from", "") or "declared"),
            "quad": [[round(x0, 1), round(y0, 1)], [round(x1, 1), round(y0, 1)],
                     [round(x1, 1), round(y1, 1)], [round(x0, 1), round(y1, 1)]],
        })
    return out
...
def quads_from_rects(rects, scale: float, factor: float) -> list[dict]:
    ...
    for rect in rects:
        ...
        quads.append({
            ...
            "ink": rect.get("ink") or "",
            # Which labelled RUN this box came out of, so a word can be tied
            # back to the whole value it is one word of -- see `page.py`'s
            # `fields` array and `record.entities_from_words`. `-1` for the
            # character grid, which measures no DOM and has no runs to index.
            "field": int(rect.get("field", -1)),
...
def _on_page(rects, number: int) -> list[dict]:
    """The measured rects that landed on printed side `number`.

    `page.py` stamps every rect with the sheet it was measured against, and a
    single-sheet page (every layout without a `page_order`, and every character
    grid) stamps 1 on all of them -- so this is the identity for them and a
    filter only where it has to be.
    """
    return [rect for rect in (rects or []) if int(rect.get("page", 1)) == number]


def clip_to_page(items: list[dict], width: int, height: int) -> list[dict]:
    """Every quad corner pulled inside the page, and anything left with no area
    dropped.

    `#sheet` is `overflow:hidden`, so ink outside it is not painted -- but
    `getBoundingClientRect()` measures the element's own geometry and knows
    nothing about the crop. A run that straddles the paper's edge therefore
    reports a box wider than the ink that survived, and `pipeline/invariants.py`
    is right to refuse it: the box claims pixels nothing painted.

    Measured on the run that found this: `seal_edge_half` is anchored
    `page_edge_left`, which centres the mark ON the left edge for the "giáp
    lai" look. `seal_mark` already halves that label box and pins it to
    `left:0%` -- and then rotates it by up to 3 degrees, which swings the
    bounding rect two pixels back past the edge. One page in two hundred, at
    the very end of a two-hour run: `boxes[0] corner (-2,874) is outside the
    1332x1883 frame`, and the shard stopped there with its images unassembled.

    Clamping is not papering over that. The clipped box is the TRUE description
    of the visible ink, which is what a label is for; a box that clipped away to
    nothing described no ink at all and has no business in the record.
    """
    out = []
    for item in items:
        quad = item.get("quad")
        if not isinstance(quad, list) or len(quad) != 4:
            out.append(item)
            continue
        pulled = [[min(max(float(x), 0.0), float(width)),
                   min(max(float(y), 0.0), float(height))] for x, y in quad]
        xs = [corner[0] for corner in pulled]
        ys = [corner[1] for corner in pulled]
        if max(xs) - min(xs) < 1.0 or max(ys) - min(ys) < 1.0:
            continue                      # clipped away to nothing: no ink, no box
        out.append({**item, "quad": pulled})
    return out
    ...
    def render(self, seed: int, force: dict[str, str] | None = None):
        ...
        try:
            with profiling.stage("render"):
                ...
                # Every printed side, in print order. One element unless the
                # layout declared a `page_order` -- see `sheets/base.py::
                # page_order` -- and the FIRST is still `#sheet`, so nothing
                # written against that id had to change.
                sheet_elements = page.query_selector_all(".sheet")
                if not sheet_elements:
                    raise RuntimeError(
                        "no .sheet element on the page; nothing to screenshot")
            ...
            with profiling.stage("render"):
                shots = [element.screenshot(type="png") for element in sheet_elements]
        finally:
            with profiling.stage("render"):
                page.close()

        # One printed side at a time, from here down. A single-page document is
        # a list of one and walks exactly the same path -- there is no second
        # branch for the common case, because a second branch is how the two
        # would come to disagree about the scale factor or the ageing seed.
        drawn: list[dict] = []
        factor = 1.0
        for number, shot in enumerate(shots, start=1):
            with profiling.stage("render"):
                ...
                # Render large and shrink, never the reverse: the text stays
                # crisp, and the result lands in the same size band as the glyph
                # renderer, so the two are comparable at the pixel level and not
                # only in content.
                #
                # The factor is drawn ONCE, off the first sheet, and reused: two
                # sides of one document printed at two different scales is not a
                # document anybody handles, and it would also make the two pages
                # of one table disagree about how wide a column is.
                if number == 1:
            ...
            with profiling.stage("geometry"):
                ...
                marks = graphics_from_rects(_on_page(graphics, number),
                                            self.scale, factor)
                # No scale, no warp, no clip: `fields` carries no geometry.
                # See `page.py`'s `fields` comment for why it must not.
                runs = _on_page(rects.get("fields") or [], number)

            # Ageing runs after the boxes are computed and must not move a pixel
            # -- every model in `degradation/` filters or composites in place.
            # Asserted rather than assumed: a resize slipped into the chain would
            # shift every box without changing anything visible about the image.
            ...
            #
            # `seed + number - 1`: each side is a separate sheet of paper that
            # went through the same machine, so it ages the same WAY and not to
            # the same pattern. Page 2 carrying page 1's exact fold is the tell
            # that would give a multi-page set away at a glance.
            page_seed = seed + number - 1
            with profiling.stage("degradation"):
                # The boxes go in as well as the image: `by_box` puts a model on
                # a few text boxes rather than on the whole sheet, and the boxes
                # are the only thing that says where those are. Computed just
                # above, so they describe THIS page rather than the one before
                # ageing shifted anything -- which is also why nothing in the
                # chain may move a pixel. See degradation/regions.py.
                aged = apply_recipe(image, recipe, seed=page_seed, boxes=boxes)
            ...
            # và header "HÌNH HỌC" của `rulebase/rules/augmentation.yaml`. Vắng
            # mặt (giá trị mặc định của mọi giá trị khác) thì bước này là no-op.
            #
            # Tên warp chọn ENGINE, qua `degradation/warp.py`: `paper_photo` là
            # trường dịch chuyển đọc từ ảnh chụp giấy (milli-giây, xem
            # `degradation/paper_warp.py`), năm tên còn lại render thật qua
            # Blender bằng subprocess (vài giây tới hơn một phút một trang).
            warp = recipe.get("augmentation", "warp")
            if warp:
                with profiling.stage("geometry"):
                    aged, boxes, words, cells, marks = warp_regions(
                        warp["name"], aged, warp.get("params"),
                        random.Random(page_seed), boxes, words, cells, marks)

            page_h, page_w = aged.shape[:2]
            drawn.append({
                "image": aged,
                "boxes": clip_to_page(boxes, page_w, page_h),
                "words": clip_to_page(words, page_w, page_h),
                "cells": clip_to_page(cells, page_w, page_h),
                "graphics": clip_to_page(marks, page_w, page_h),
                "fields": runs,
            })

        return (recipe, receipt, grid, drawn, hand_report, sign_report, markup)
...
def _emit_page(args, name, recipe, receipt, drawn, hand_report, sign_report,
               markup, seed, notes) -> None:
    ...
    with profiling.stage("annotation"):
        ...
        cells = [cell for page in drawn for cell in page["cells"]]
        if cells:
        ...
        if len(drawn) > 1:
            extra["pages"] = names
        # ONE record for the document, however many sides it took: `pages[]`
        # numbers them and `source_files` names their images. The nested label
        # (`ground_truth`) is the document's, not any one side's -- a total that
        # printed on page 2 is still the same document's total.
        item = record.build(
            filename=name, width=drawn[0]["image"].shape[1],
            height=drawn[0]["image"].shape[0],
            parser="html", ink=page_ink(recipe),
            extracted=receipt.ground_truth(), seed=seed, layout=recipe.layout.id,
            sheets=[{"filename": page_name,
                     "width": page["image"].shape[1],
                     "height": page["image"].shape[0],
                     "boxes": page["boxes"], "words": page["words"],
    ...
    with profiling.stage("export"):
        ...
        # One provenance line per IMAGE, because the caller counting images is
        # counting files on disk -- but the same `job_id` on all of them, since
        # they are one document and one record.
        for number, page_name in enumerate(names, start=1):
            note = dict(extra) if number == 1 else {**extra, "page_of": name,
                                                   "page_number": number}
            notes.add(page_name, job_id=item["job_id"], layout=recipe.layout.id,
                      recipe=recipe.to_dict(),
                      text_sequence=receipt.text_sequence(), extra=note)
            ...
            for index, job, seed in worklist.pages(jobs):
                ...
                with clock.time(name, layout=job.layout or "") as timed:
                    ...
                    (recipe, receipt, _grid, drawn, hand_report, sign_report,
                     markup) = renderer.render(seed, forces[job])
                    ...
                    _emit_page(args, name, recipe, receipt, drawn,
                ...
                first = drawn[0]["image"]
                boxes = [box for page in drawn for box in page["boxes"]]
                print(f"[ok] {name}  {first.shape[1]}x{first.shape[0]}  "
                      + (f"{len(drawn)} trang  " if len(drawn) > 1 else "")
