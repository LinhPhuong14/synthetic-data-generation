...
# CHỮ KÝ: mọi mặt chữ trong `fonts/hand/`, đọc từ thư mục.
#
# Hai mặt chữ là hai chữ ký. Cả một bộ dữ liệu hai mươi nghìn tờ mà chỉ có hai
# nét chữ ký thì mô hình học được "chữ ký trông như PatrickHand hoặc
# IndieFlower", không học được "chữ ký là nét tay của một người".
#
# Đọc thư mục chứ không liệt kê: `notebook.py` cố ý liệt kê vì trang vở của nó
# phải trông y hệt qua từng lượt chạy, còn ở đây đa dạng CHÍNH LÀ mục tiêu.
# `sorted()` giữ tính định: cùng thư mục, cùng seed thì cùng chữ ký.
#
# Ba mươi mặt chữ thêm vào lấy từ Google Fonts (giấy phép OFL), lọc bằng
# `cmap` thật: mỗi mặt phải phủ đủ 59 ký tự tiếng Việt không có trong Latin-1.
# Trong 99 họ viết tay Google Fonts khai là có subset `vietnamese`, chỉ một
# phần phủ đủ khi kiểm bảng mã -- khai có không bằng có thật.
# `FACES[0]` là mặt chữ MẶC ĐỊNH của `Ink()`, nên thứ tự ở đây có tải trọng.
# Sắp theo chữ cái thuần thì `AlexBrush` lên đầu -- một script bay bướm có chữ
# `l` chỉ cao 0,96 lần chiều cao x, và `test_a_tall_letter_reaches_above_the_x_height`
# bắt được ngay. Chữ ký bay bướm không sai; sai là lấy nó làm mặc định.
DEFAULT_FACES = ("PatrickHand-Regular.ttf", "IndieFlower-Regular.ttf")


def _faces() -> tuple[str, ...]:
    found = sorted(p.name for p in HAND_FONT_DIR.glob("*.ttf"))
    first = [f for f in DEFAULT_FACES if f in found]
    # Lưới đỡ: thư mục trống thì vẫn phải có một mặt chữ để `svg()` chạy.
    return tuple(first + [f for f in found if f not in first]) or DEFAULT_FACES
