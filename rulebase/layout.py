...
# ---------------------------------------------------- nhiều tờ cho một chứng từ

# The one block name allowed to appear on more than one sheet: an item table
# running past the bottom of the first is a continuation, and every other name
# repeated is that block drawn twice.
TABLE_SECTION = "table"


# Which CSS families in `generators/html/sheets/` actually READ `page_order`.
# A layout that declares one in any other family draws the whole document on a
# single sheet and says nothing -- the silent short page this key exists to
# prevent. Named here, beside the rule, so the two places that must agree --
# `agent/compose_layout.py` writing a layout and `tests/test_sheets.py`
# checking the committed ones -- read the same list instead of each keeping a
# copy. Wiring a family is a deliberate act; adding it here is how it is said.
PAGINATING_FAMILIES = ("form", "medical", "modern")


class PageOrderError(ValueError):
    """`page_order` names a page nobody can print."""


def page_order(spec: dict, sections) -> list[list[str]]:
    """`page_order` as one list of block names per sheet. `[]` means one sheet.

    A cut sheet's height is decided before anything is printed, so a document
    with more content than one sheet holds is TWO SHEETS, not one tall sheet --
    and which block starts the second one is a decision a printer makes, not an
    arithmetic consequence of where the first ran out. So it is declared:

        sections:   [letterhead, doctitle, parties, table, totals, signatures]
        page_order:
          - [letterhead, doctitle, parties, table]
          - [table, totals, signatures]

    Read that as: the item table runs across BOTH sheets (named twice, which is
    the only name allowed to repeat), the totals and the signature block wait
    for the second. Absent, this returns `[]` and every caller draws the single
    sheet it always drew -- which is what keeps every committed layout, and
    every fingerprint in `tools/baseline.py`, unchanged.

    Here rather than in `generators/html/sheets/base.py` because two callers
    need the same answer: the family drawing the page, and `agent/compose_
    layout.py` checking a composition before it is written to a file. A rule
    about what a layout FILE may say belongs with the other facts about layout
    files.

    Refused rather than repaired, because each of these is a layout that would
    otherwise print a page silently missing something:

    * a name that is not in `sections` -- nothing draws it, so the page is
      short by exactly the block the author was thinking of;
    * a section left out of every page -- same result, from the other end;
    * any repeat other than the table -- two signature blocks on one document
      is not a continuation, it is the block drawn twice.
    """
    raw = spec.get("page_order")
    if not raw:
        return []
    if not isinstance(raw, list) or not all(isinstance(page, list) for page in raw):
        raise PageOrderError("page_order must be a list of lists of block names")
    pages = [[str(name) for name in page] for page in raw if page]
    if len(pages) < 2:
        # One page is the ordinary sheet, spelled the long way. Not an error --
        # a variant that dropped a page should still draw.
        return []
    known = list(sections)
    seen: dict[str, int] = {}
    for page in pages:
        for name in page:
            if name not in known:
                raise PageOrderError(
                    f"page_order names {name!r}, which is not in sections: "
                    f"{', '.join(known)}")
            seen[name] = seen.get(name, 0) + 1
    repeated = sorted(name for name, count in seen.items()
                      if count > 1 and name != TABLE_SECTION)
    if repeated:
        raise PageOrderError(
            f"only {TABLE_SECTION!r} may run across pages; page_order repeats "
            f"{', '.join(repeated)}")
    missing = [name for name in known if name not in seen]
    if missing:
        raise PageOrderError(
            f"page_order leaves {', '.join(missing)} on no page at all")
    return pages
...
__all__ = [
    "PAGINATING_FAMILIES", "PageOrderError", "TABLE_SECTION",
    "page_order",
