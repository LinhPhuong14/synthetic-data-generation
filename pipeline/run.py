        ...
        with synthesis.Writer(synthesis.beside(target), backend) as notes, \
            ...
            for shard in sorted((s for s in plan["shards"] if s["backend"] == backend),
                ...
                for item in record.read(directory):
                    ...
                    # EVERY page, not just the first. A multipage document is
                    # one record over N images and names them all in
                    # `source_files`; linking only `file_name` left the sheets
                    # after the first behind in the shard, and the record then
                    # pointed at an image the dataset did not contain.
                    for page_name in (item.get("source_files") or [name]):
                        page_source = directory / str(page_name)
                        if not page_source.exists():
                            warnings.append(
                                f"{page_name}: named by {name}'s record but not "
                                f"in shard {shard['index']}")
                            continue
                        page_destination = target / str(page_name)
                        try:
                            os.link(page_source, page_destination)
                        except OSError:
                            shutil.copy2(page_source, page_destination)
