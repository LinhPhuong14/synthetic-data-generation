...
def words_from_boxes(boxes: Iterable[dict[str, Any]],
    ...
    for index, box in enumerate(boxes or []):
        ...
        out.append({
            ...
            # Axis 4. See `field_type_for`. `box.get("shape")` is empty for
            # almost every word -- only the one question span a `shape` is
            # tagged on (`generators/html/page.py`'s `fields` array, not
            # `words`/`cells`) ever carries it -- so most words resolve by
            # `kind` alone, same floor `entities_from_words` falls back to.
            "field_type": field_type_for(kind, str(box.get("shape", ""))),
            "field_path": kind,
            "relation_path": kind,
            "field_pattern": kind,
