...
def render_shard(shard: dict, out: Path, plan: dict, *, rules_root: Path | None = None,
    ...
    with synthesis.Writer(synthesis.beside(directory), backend) as notes:
        try:
            ...
            # MỌI ảnh các bản ghi gọi tên, không chỉ tờ đầu của mỗi bản ghi.
            #
            # `record.read` trả một bản ghi cho một CHỨNG TỪ, còn một chứng từ
            # hai tờ có hai ảnh và hai dòng provenance (`sheets/base.py::
            # page_order`). Đếm bằng `file_name` là đếm tờ đầu, nên tờ sau hiện
            # ra dưới dạng "synthesis entry names no image" và cả shard 25 tờ
            # bị bỏ — đo được trên lượt chạy này: `first 'html_002_p2.jpg'`.
            gaps = drew.problems(
                name for item in produced
                for name in (record.source_files(item) or [record.file_name(item)]))
            if gaps:
                raise ShardError(
            ...
            for run in shard["runs"]:
                for offset in range(run["count"]):
                    ...
                    # Every printed side of this document, staging name to
                    # dataset name. One entry for all but a multi-page layout
                    # (`sheets/base.py::page_order`), where the record already
                    # names page 2 and page 3 -- and the record is the ONLY
                    # thing that does, so a move that took `drawn_name` alone
                    # would leave them in staging to be deleted with it, with
                    # a record naming two images and a directory holding one.
                    staged_pages = [str(each) for each
                                    in (item.get("source_files") or [drawn_name])]
                    page_targets = record.page_names(target, len(staged_pages))
                    for staged_name, page_target in zip(staged_pages, page_targets):
                        shutil.move(str(staging / staged_name),
                                    str(directory / page_target))
