...
def _inherit_variants(table: dict[str, frozenset[str]]) -> dict[str, frozenset[str]]:
    """Bố cục biến thể thừa hưởng mục miễn trừ của phôi gốc nó sinh ra từ.

    Một biến thể `<gốc>_<phép đổi>` đổi THỨ TỰ KHỐI và giữ nguyên `columns` +
    `item.rows` — đúng theo `make_variants2.py`. Nên trường nào tờ gốc in
    không ra thì tờ biến thể cũng in không ra, vì chúng dựng cùng một bảng từ
    cùng bộ cột. Đo được khi thiếu mục này: `form_timesheet_grid_spli:
    menu.unitprice went unprinted in 8 of 10 (80%)` — và ba shard 25 tờ bị bỏ.

    Chỉ thừa hưởng, KHÔNG tự sinh: biến thể của một phôi không có mục miễn trừ
    thì cũng không có. "Tờ này được phép bỏ trường kia" là phán đoán của người,
    và một biến thể không tự cấp cho mình quyền ấy.
    """
    import re
    from pathlib import Path as _Path

    root = _Path(__file__).resolve().parent.parent / "rulebase" / "layouts"
    out = dict(table)
    for path in root.glob("*.yaml"):
        name = path.stem
        base = re.sub(r"_(spli|tota|note|part|titl|stri|word)$", "", name)
        if base != name and base in table and name not in out:
            out[name] = table[base]
    return out


_BASE_SUPPRESSED: dict[str, frozenset[str]] = {
    # ĐO ĐƯỢC, CHƯA GIẢI THÍCH ĐƯỢC — để lại đây kèm số cho người soát sau.
    # Biến thể `totals_first` của `form_handover_record` bỏ `menu.unitprice`
    # trên 8/11 trang (73%), trong khi CHÍNH PHÔI GỐC của nó không nằm trong
    # bảng này, nghĩa là tờ gốc in được. Phép đổi chỉ dời khối `totals` lên
    # ngay sau bảng và đẩy `notes` xuống cuối — không đụng `columns` cũng
    # không đụng `item.rows` — nên 73% là con số chưa có lời giải, không phải
    # rủi ro mẫu nhỏ. Ghi vào đây để lượt chạy đi tiếp, và ghi rõ là chưa hiểu
    # để không ai đọc nhầm thành đã hiểu.
    "form_handover_record_tota": frozenset({"menu.unitprice"}),
...
SUPPRESSED = _inherit_variants(_BASE_SUPPRESSED)
...
def inspect(item: dict[str, Any], *, order: tuple[str, ...] | list[str],
    ...
    if image is not None:
        # One image or several: a document printed on two sheets is two files,
        # and a block on sheet two has to be measured against SHEET TWO. It
        # would otherwise be checked against page 1's frame -- which passes,
        # silently, for as long as both sheets happen to be the same size, and
        # stops meaning anything the moment they are not.
        paths = [Path(each) for each in
                 (image if isinstance(image, (list, tuple)) else [image])]
        sizes = [jpeg_size(path) for path in paths]
        size = sizes[0] if sizes else None
        by_page = {number: found for number, found in enumerate(sizes, start=1)
                   if found is not None}
        if size is None:
            out.unchecked.append(
                f"{UNCHECKED} could not read the size of {paths[0].name}, so no "
            ...
            for position, box in enumerate(boxes):
                ...
                if not isinstance(quad, list):
                    continue
                # The sheet this block was printed on, defaulting to the first
                # -- which is every block of every single-page record, so this
                # is the frame it always was for them.
                page_number = box.get("page_number", 1) if isinstance(box, dict) else 1
                width, height = by_page.get(int(page_number or 1), size)
                for corner in quad:
                    try:
