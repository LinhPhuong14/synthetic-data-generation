...
def sheet_overflow(seeds: int = SHEET_SEEDS) -> list[str]:
    ...
    """
    problems: list[str] = []
    # Only the layouts a run can actually draw. `available_layouts()` lists the
    # FILES, and a file outlives its rule: switching a layout off switches its
    # document off with it (they gate each other through one tag), so pinning
    # the layout alone leaves the sampler with no document that produces the
    # tag it needs and `sample_recipe` raises on every seed -- see
    # `tests/conftest.py::force_for`, which exists for exactly this. Measured
    # the day `newspaper_classifieds` was retired: preflight stopped the whole
    # run with `RuleError: layout='newspaper_classifieds' cannot be drawn at
    # all`, which is true, expected, and none of this check's business.
    from rulebase.spec import load_rules as _load_rules

    drawable = {option.id for option in _load_rules().get("layout", ())
                if option.enabled and option.weight > 0}
    for layout_id in rulebase.available_layouts():
        if layout_id not in drawable:
            continue
        ...
        for seed in range(seeds):
            ...
            # A layout with a `page_order` is printed on that many sheets, so
            # its ceiling is that many sheets high (see `sheets/base.py::
            # page_order`). Measuring its whole content against ONE sheet is
            # how a correctly paginated layout would be reported as overflowing
            # the paper it was explicitly split to fit.
            sides = max(len(spec.get("page_order") or []), 1)
            over = content / (sides * width_px / ratio)
            if over > worst:
                worst, worst_seed = over, seed
        if worst > OVERFLOW:
            problems.append(
                f"{layout_id}: content is {worst:.0%} of the "
                + (f"{len(spec.get('page_order') or [])} {grid.sheet} sheets"
                   if spec.get("page_order") else f"{grid.sheet} sheet")
                + f" it declares (seed {worst_seed}), so the page grows past its paper"
            )
    return problems
