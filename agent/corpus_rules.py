...
def _punctuation() -> frozenset[str]:
    """Dấu câu corpus THẬT SỰ dùng, đo từ chính nó.

    Bản trước là một tập viết tay mười sáu ký tự, và nó thiếu em-dash -- nên
    luật chê `Phòng Hành chính — Nhân sự`, một dòng người ta đã viết và đã
    commit. Docstring của `test_the_rules_accept_every_committed_line` nói
    đúng cái cần nói: *một luật chê dòng người ta đã viết là một luật hỏng*.

    Đo thì hết hỏng: corpus dùng đúng mười bảy dấu, và mười bảy dấu ấy là
    định nghĩa. Thêm một dòng có dấu mới thì phải sửa corpus trước -- đó là
    thứ tự đúng, vì corpus là cái người ta viết còn luật là cái ta suy ra.

    Vẫn chê được cái đáng chê: Cyrillic, CJK, emoji không có trong corpus nên
    không vào tập này. Phép kiểm chữ cái vẫn đòi gốc LATIN.

    Cùng lối `agent/layout_schema.py::charset` đã dùng cho bố cục -- đo bộ ký
    tự thay vì khai nó."""
    import unicodedata as _ud                                 # noqa: PLC0415

    found: set[str] = set()
    root = Path(__file__).resolve().parents[1] / "rulebase" / "corpus"
    for path in sorted(root.rglob("*.txt")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.startswith("#"):
                continue
            for ch in line:
                if ch.isspace() or ch.isdigit() or _ud.combining(ch):
                    continue
                base = _ud.normalize("NFD", ch)[0]
                if not _ud.name(base, "").startswith("LATIN"):
                    found.add(ch)
    # Lưới đỡ: corpus mất thì đừng chê mọi dòng có dấu ngoặc.
    return frozenset(found) or frozenset("!\"%&'()+,-./:;[]")
