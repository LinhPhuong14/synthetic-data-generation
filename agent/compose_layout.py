...
from rulebase import layout as layout_mod
...
class Vocabulary:
    ...
    aligns: tuple[str, ...]
    title_max: int
    # The whole-page keys -- `page.style`, `page.sidebar_sections` and the two
    # decorations. NOT a section: `page` is the only key of a layout that says
    # what SHAPE the sheet is rather than what goes in the next block down, and
    # it was the one derived group `vocabulary()` used to drop on the floor
    # (`head not in SECTIONS`). Dropping it capped every composed layout at one
    # vertical stack, while three hand-written layouts had been drawing a
    # sidebar all along -- a ceiling nobody chose, in a file whose whole job is
    # to widen the space.
    page: dict[str, dict] = field(default_factory=dict)
    # Whether a composition may declare `page_order` -- true once a committed
    # layout does, false before that, because the derivation is what says a key
    # exists at all. See `vocabulary()`.
...
def vocabulary(schema: dict[str, schema_mod.Leaf] | None = None) -> Vocabulary:
    ...
    # `page.*`, same derivation, one level up. A `[]` here means a LIST of that
    # leaf, and the two kinds have to be told apart: `page.sidebar_sections[]`
    # is a list of block names of any length, while `<section>.<leaf>[]` above
    # is the two-number range `randrange` is fed. Reading them the same way
    # would offer the model a sidebar of exactly two blocks.
    page: dict[str, dict] = {}
    for path, leaf in schema.items():
        head, _, rest = path.partition(".")
        if head != "page" or not rest or "." in rest.rstrip("[]"):
            continue
        name, listed = (rest[:-2], True) if rest.endswith("[]") else (rest, False)
        spec = _leaf_spec(leaf)
        if spec is None:
            continue
        page[name] = ({"type": "array", "items": spec, "minItems": 1,
                       "uniqueItems": True} if listed else spec)

    # `page_order[][]` -- a list of lists of block names. Derived like every
    # other key, so a composition may declare pages exactly once a hand-written
    # layout has shown what the key looks like.
    paginates = "page_order[][]" in schema
...
class Composition:
    ...
    # The shape of the sheet, not the contents of a block -- `page.style:
    # sidebar` puts the named blocks in a rail down the side and the rest in
    # the main column. Empty is the ordinary single stack, and stays the
    # common answer: a rail is right for a statement or a certificate, wrong
    # for a till roll 80 mm across.
    page: dict[str, Any] = field(default_factory=dict)
    # Which blocks go on which printed side, when the document needs more than
    # one. Empty is the single sheet, and stays the common answer.
    page_order: list[list[str]] = field(default_factory=list)
...
def _whole(value: float, way: str) -> int:
    """Cận của một nút `integer` phải là SỐ NGUYÊN.

    `leaf_schema` đã làm tròn cận của mình (xem chú thích ở đó: một máy chủ
    vLLM thật trả HTTP 500 không kèm lời nào khi bộ giải mã dựng lexeme số
    nguyên từ `minimum: 1.4285714285714286`). Nhưng ba nút `integer` khác --
    `width` của cột, `width` của khổ giấy, `gutter` -- lấy cận thẳng từ
    `limits`, và `limits` phái sinh từ dữ liệu của từng chứng từ. Đổi một dòng
    trong một file YAML là đủ để một cận hoá phân số.

    `down` cho cận dưới, `up` cho cận trên: nới ra phía ngoài, nên không giá
    trị nguyên nào đang hợp lệ bị loại đi."""
    import math                                                # noqa: PLC0415

    return math.floor(value) if way == "down" else math.ceil(value)
...
def schema_for(document: str, limits, layers_: Layers, vocab: Vocabulary,
    ...
    column = {
        ...
        "properties": {
            ...
            "width": {"type": "integer", "minimum": 0,
                      "maximum": _whole(_column_ceiling(refs, limits), "up")},
    ...
    # Tiêu đề hai tầng: một dải chữ chạy ngang mấy cột cùng trả lời một câu
    # hỏi ("Nguồn thanh toán (đồng)" trên bốn cột tiền của bảng kê viện phí).
    # `components/table.py` đã dựng được từ lâu và `medical_statement.yaml`
    # đang dùng; chỗ thiếu là composer không KHAI được, vì `vocabulary()` bỏ
    # mọi khoá lồng và `table.header_groups[]` là một danh sách các object.
    # Khai thẳng ở đây thay vì nới bộ dẫn xuất: cái này có hình dạng riêng
    # (`from`/`to` phải là KHOÁ CỘT, không phải một chuỗi bất kỳ), và một
    # schema dẫn xuất chỉ nhìn thấy "list of dict" thì không nói được điều đó.
    header_group = {
        "type": "object",
        "properties": {
            "title": {"type": "string", "minLength": 2, "maxLength": vocab.title_max},
            "from": {"type": "string", "enum": keys},
            "to": {"type": "string", "enum": keys},
        },
    ...
    block = {
        ...
        "properties": {
            ...
            # Tối đa hai dải: ba dải trên một bảng là bảng cần tách làm hai.
            "header_groups": {"type": "array", "items": header_group,
    ...
    return {
        ...
        "properties": {
            "paper": {
                ...
                "properties": {
                    ...
                    "width": {"type": "array", "minItems": 2, "maxItems": 2,
                              "items": {"type": "integer",
                                        "minimum": _whole(limits.width[0], "down"),
                                        "maximum": _whole(limits.width[1], "up")}},
                    "gutter": {"type": "integer",
                               "minimum": _whole(limits.gutter[0], "down"),
                               "maximum": _whole(limits.gutter[1], "up")},
                },
            ...
            "settings": settings,
            # Optional on purpose: a schema that REQUIRED `page` would make
            # every composed layout answer a question most documents do not
            # ask, and "no page key" is how all but three of the committed
            # layouts spell the ordinary stack.
            **({"page": {"type": "object", "properties": dict(vocab.page),
                         "additionalProperties": False}} if vocab.page else {}),
            # Optional, like `page`, and for the same reason: most documents fit
            # one sheet, and "no page_order" is how all but one of the committed
            # layouts say so. `minItems: 2` because one page is not a page order.
            **({"page_order": {
                "type": "array", "minItems": 2, "maxItems": limits.sections_max,
                "items": {"type": "array", "minItems": 1,
                          "items": {"type": "string", "enum": sorted(available)}},
            }} if vocab.paginates else {}),
...
def user_message(document: str, limits, refs: list[Reference], layers_: Layers,
    ...
    if vocab.page:
        style = (vocab.page.get("style") or {}).get("enum") or []
        rail = ((vocab.page.get("sidebar_sections") or {}).get("items") or {}).get("enum") or []
        lines[-1:-1] = [
            "### Dáng tờ giấy (`page`) — khoá DUY NHẤT nói tờ này không phải "
            "một chồng khối dọc",
            "",
            "Bỏ hẳn `page` là tờ giấy xếp dọc từ trên xuống, và **phần lớn "
            "chứng từ đúng là thế**. Nhưng có tờ mà thông tin định danh chạy "
            "dọc một dải hẹp bên cạnh còn phần thân giữ nguyên bề ngang: sao "
            "kê ngân hàng, giấy chứng nhận, trang báo. Với những tờ ấy, một "
            "chồng dọc là *sai dáng*, không phải là *dáng khác*.",
            "",
            f"* `page.style`: {', '.join(repr(v) for v in style) or '—'}",
            f"* `page.sidebar_sections`: khối nào xuống dải bên — chọn trong "
            f"{', '.join(rail) or '—'}",
            "",
            "Ba luật, và cả ba đều là lý do một dải bên hỏng chứ không phải "
            "quy ước:",
            "",
            "1. **Chỉ đặt `sidebar_sections` khi `style` là `sidebar`.** Không "
            "có dải thì danh sách ấy không có chỗ để đi.",
            "2. **Mọi tên trong `sidebar_sections` phải là khối bạn ĐÃ đưa vào "
            "`blocks`.** Tên không có trong `blocks` là một chỗ trống trên dải, "
            "không phải một khối mới.",
            "3. **Không đẩy hết mọi khối xuống dải.** Cột chính rỗng thì đó "
            "không còn là tờ có dải bên, đó là tờ giấy hẹp. Bảng hàng "
            "(`table`) và khối tổng (`totals`) ở lại cột chính — chúng cần bề "
            "ngang, và đó chính là thứ dải bên không có.",
            "",
        ]
    if vocab.paginates:
        lines[-1:-1] = [
            "### Mấy tờ giấy (`page_order`) — chỉ khai khi tờ giấy THẬT SỰ "
            "không đủ chỗ",
            "",
            "Khổ giấy cắt sẵn có chiều cao quyết định trước khi in. Một chứng "
            "từ nhiều nội dung hơn một tờ chứa được là **hai tờ**, không phải "
            "một tờ cao gấp đôi — và khối nào mở đầu tờ thứ hai là quyết định "
            "của nhà in, không phải hệ quả số học của chỗ tờ đầu hết giấy.",
            "",
            "```",
            "page_order:",
            ...
            "```",
            "",
            "* Mỗi khối trong `blocks` phải nằm trên **đúng một** tờ — trừ "
            "`table`.",
            "* **`table` là tên DUY NHẤT được lặp.** Lặp nghĩa là bảng hàng "
            "chạy tiếp sang tờ sau: tiêu đề cột in lại, số dòng chạy tiếp chứ "
            "không quay về 1. Mọi tên khác lặp là in khối ấy hai lần.",
            "* Khối nào không có tờ nào nhận là khối không được in. Cả bố cục "
            "bị từ chối, không phải bị bỏ qua.",
            "* Bỏ hẳn `page_order` là một tờ — và **phần lớn chứng từ đúng là "
            "thế**. Hoá đơn ba dòng hàng chia làm hai tờ để tờ sau đỡ trống là "
            "tờ giấy không ai từng cầm.",
            "",
            "Số dòng hàng quyết định có tờ hai hay không, còn `page_order` chỉ "
            "nói *nếu có thì khối nào xuống đó*. Nên khai nó cho một chứng từ "
            "mà bảng hàng thường dài — bảng kê, phiếu xuất kho nhiều mặt hàng "
            "— chứ không phải cho một biên nhận.",
            "",
        ]
...
def to_layout(comp: Composition, layout_id: str, limits, refs: list[Reference],
    ...
    * **`family:`** and `rules:` -- inherited from the references, because the
    ...
    # The page shape, kept honest against the blocks that are actually on the
    # page. A `sidebar_sections` naming a block the composition never drew
    # gives a rail with a gap in it and a main column missing nothing -- the
    # builder reads the list as a SET MEMBERSHIP test (`sheets/modern.py`
    # walks `sections` and asks which side each one goes), so an absent name
    # is silently nothing rather than an error. And a rail with every block in
    # it leaves the main column empty, which is not a sidebar, it is a narrow
    # page. Both are repaired here rather than refused: the shape the model
    # asked for is still the shape it gets, minus the names it could not have.
    page = {name: value for name, value in comp.page.items()
            if name in vocab.page}
    if page.get("style") == "sidebar":
        rail = [name for name in dict.fromkeys(page.get("sidebar_sections") or ())
                if name in comp.sections]
        if not rail or len(rail) >= len(set(comp.sections)):
            page.pop("style", None)
            page.pop("sidebar_sections", None)
        else:
            page["sidebar_sections"] = rail
    else:
        page.pop("sidebar_sections", None)
    if page:
        spec["page"] = page

    # The page order, held against the SAME rule the renderer applies -- one
    # function, in `rulebase/layout.py`, so a composition cannot be written
    # that the family would then refuse. Dropped rather than raised when it
    # does not hold: the composition is still a layout, just a one-sheet one,
    # and a run that threw away six good blocks over a bad page split would
    # be trading more than it gained.
    if comp.page_order and family in layout_mod.PAGINATING_FAMILIES:
        try:
            pages = layout_mod.page_order({"page_order": comp.page_order},
                                          spec["sections"])
        except layout_mod.PageOrderError:
            pages = []
        if pages:
            spec["page_order"] = pages
    ...
    if table is not None:
        ...
        # Dải tiêu đề gộp, giữ đúng bốn điều kiện `_header_rows` cần để dựng ra
        # một dải thật. Vá chứ không từ chối cả bố cục: một dải hỏng là một dải
        # bỏ đi, còn tám khối kia vẫn là một tờ giấy dùng được.
        #
        #   * `from`/`to` phải là cột CÓ TRÊN BẢNG NÀY — schema chỉ ràng được
        #     chúng thuộc danh sách khoá của chứng từ, không ràng được bảng vừa
        #     soạn có dùng khoá ấy không;
        #   * `from` phải đứng trước `to`, vì `_header_rows` lấy `keys.index()`
        #     hai đầu rồi trải colspan giữa chúng — ngược thì span âm;
        #   * dải một cột không phải dải gộp, nó là cái tiêu đề cột đã có;
        #   * hai dải không được chồng nhau: một cột chỉ nằm dưới một câu hỏi.
        order = [c["key"] for c in columns]
        groups, taken = [], set()
        for entry in (table.get("header_groups") or []):
            ...
            if hi <= lo or set(range(lo, hi + 1)) & taken:
                continue
            taken |= set(range(lo, hi + 1))
            groups.append({"title": str(entry.get("title", "")).strip(),
                           "from": first, "to": last})
        # `item.rows` CHÉP TỪ PHÔI, không dựng bằng ánh xạ đồng nhất.
        #
        # Ánh xạ đồng nhất (`{col: k, from: k}`, một dòng) đúng với họ `modern`,
        # nơi tên cột trùng tên trường -- và sai với mọi họ khác. `market_
        # barcode.yaml` có cột `name` nhưng đổ `from: barcode` vào đó, rồi đẩy
        # tên hàng xuống DÒNG THỨ HAI. Dựng lại bằng ánh xạ đồng nhất là mất
        # `menu.barcode`, và cửa ải 6 bắt đúng ngay bố cục đầu tiên của họ ấy:
        #
        #     seed 0: menu.barcode '9489241157815' is in the label and on no run
        #     seed 0: menu.nm 'MIẾN DONG TÀI KÝ 300G' is in the label and on no run
        #
        # Phôi là chỗ duy nhất biết dữ liệu của chứng từ này chảy vào cột nào,
        # nên nó là chỗ chép. Chỉ lọc bỏ ô nào trỏ tới cột bố cục mới không
        # dùng; ô còn lại giữ nguyên `from`, kể cả khi `from` khác `col`.
        chosen = {c["key"] for c in columns}
        template = [[dict(cell) for cell in row
                     if str(cell.get("col")) in chosen]
                    for row in ((primary.get("item") or {}).get("rows") or [])]
        template = [row for row in template if row]
        spec["item"] = {
            "wrap_name": bool(primary.get("item", {}).get("wrap_name", True)),
            # Phôi không khai `item.rows` thì mới về ánh xạ đồng nhất -- đó là
            # trường hợp cột và trường thật sự cùng tên.
            "rows": template or [[{"col": c["key"], "from": c["key"]}
                                  for c in columns]],
        }
    ...
    # SAU vòng trên, không phải trước: vòng ấy gán `spec["table"] = options` và
    # sẽ xoá sạch dải gộp nếu nó được đặt sớm hơn. Đây là chỗ duy nhất cả hai
    # thứ -- settings của khối và dải gộp -- đã có mặt cùng lúc.
    if groups:
        spec.setdefault("table", {})
        spec["table"]["header_groups"] = groups
    return spec
...
def propose(llm, document: str, limits, refs: list[Reference], layers_: Layers,
    ...
    # Ba file, ba việc khác nhau: `compose.md` là cách LÀM (năm câu hỏi, ba
    # lớp, luật cứng), `anatomy.md` là cái BIẾT (chứng từ hành chính Việt Nam
    # cấu tạo ra sao), `regions.md` là khung phân loại người đọc sẽ gán về sau.
    # `anatomy.md` đứng giữa vì nó là thứ model phải mang sẵn trong đầu TRƯỚC
    # khi nhìn phôi -- nó tồn tại chính để phôi thôi là nguồn tự do duy nhất.
    ...
    return Composition(
        ...
        page=dict(answer.get("page") or {}),
        page_order=[[str(name) for name in page]
                    for page in (answer.get("page_order") or [])
...
def offline(document: str, limits, refs: list[Reference], layers_: Layers,
    ...
    # MỌI khoá cột bất kỳ phôi nào dùng, không bốc bỏ cái nào.
    #
    # Bốc bỏ từng cột từng là cách tạo biến thể ở đây (`rng.random() < 0.4` trên
    # nhóm "chỉ vài phôi có"). Nó sai, và cửa ải 6 bắt đúng ngay bố cục thứ hai:
    #
    #     seed 0: menu.barcode '9489241157815' is in the label and on no run
    #
    # Nội dung KHÔNG sinh theo bố cục, nó sinh theo LOẠI CHỨNG TỪ. Cửa hàng
    # tiện lợi thì mặt hàng nào cũng có mã vạch, dù bố cục có in cột ấy hay
    # không -- `rulebase/content.py` điền `menu.barcode` vì document nói thế,
    # rồi `ground_truth()` khai nó. Bỏ cột đi là nhãn hứa một giá trị không có
    # trên giấy, đúng thứ cả kho này dựng lên để chặn.
    #
    # Biến thể của bảng vì thế đến từ chỗ khác, và vẫn còn nhiều: thứ tự cột,
    # tiêu đề cột, bề ngang từng cột, có dải tiêu đề gộp hay không, và mọi cờ
    # trong `table.*`. Một nhà in khác đổi những thứ ấy; nó không bỏ bớt dữ
    # liệu của tờ giấy.
    wanted = set(layers_.key_always) | set(layers_.key_sometimes)
    ...
    # Tiêu đề hai tầng, ở chế độ bốc.
    #
    # `propose()` để model tự quyết dải gộp và §2.2 của `anatomy.md` dạy nó khi
    # nào nên có. Chế độ bốc không đọc prompt, nên nếu chỉ nối bên ấy thì mọi
    # bố cục soạn offline đều ra bảng một tầng -- đo được: 0/2 bố cục đầu tiên
    # có `header_groups`.
    #
    # Cột nào gộp thì KHÔNG bốc: lấy dải cột căn PHẢI cuối bảng. Căn phải là
    # cột số, và một dải gộp tồn tại để nói mấy con số ấy cùng trả lời một câu
    # hỏi ("Nguồn thanh toán", "Kỳ này / Luỹ kế") -- gộp một cột chữ với một
    # cột số là dải không có nghĩa. Suy từ `align` của chính các cột chứ không
    # từ một danh sách khoá viết cứng, nên khoá tiền mới thêm sau tự vào.
    header_groups: list[dict[str, str]] = []
    numeric = [i for i, c in enumerate(columns) if c["align"] == "right"]
    run = []
    for index in numeric:                      # dải LIÊN TỤC cuối cùng
        run = run + [index] if run and index == run[-1] + 1 else [index]
    if len(columns) >= 5 and len(run) >= 2 and rng.random() < 0.35:
        title = rng.choice(("Thành tiền (đồng)", "Số tiền (đồng)",
                            "Giá trị (đồng)", "Thanh toán (đồng)"))
        header_groups = [{"title": title,
                          "from": columns[run[0]]["key"],
                          "to": columns[run[-1]]["key"]}]
    ...
    for name in order:
        ...
        if name == "table" and columns:
            ...
            if header_groups:
                block["header_groups"] = header_groups
        blocks.append(block)
    ...
    # The page shape, drawn from EVIDENCE rather than from taste: a rail is
    # offered only where a phôi of this very document already carries one, so
    # the offline mode cannot invent a shape for a document whose printers
    # never use it -- the same rule the three layers put on everything else.
    # Kept to one in four, because a rail is the exception on paper too.
    railed = [ref for ref in refs if (ref.spec.get("page") or {}).get("style")]
    page: dict[str, Any] = {}
    if railed and rng.random() < 0.25:
        evidence = railed[rng.randrange(len(railed))].spec.get("page") or {}
        rail = [name for name in (evidence.get("sidebar_sections") or ())
                if name in order]
        if rail and len(rail) < len(set(order)):
            page = {"style": str(evidence["style"]), "sidebar_sections": rail}
    ...
    return Composition(
        ...
        blocks=blocks, settings=settings, page=page, by="coverage")
...
def _seed_options(section: str, refs: list[Reference], spine: Reference,
    ...
    """
    # A section name is not always a settings MAP in the file. `columns` and
    # `items` are section names in `rulebase.layout.SECTIONS` AND top-level keys
    # holding LISTS -- the column definitions and the item-row template -- and
    # `dict(a list of dicts)` raises rather than returning nothing. It surfaced
    # the day level 3 reached the till family, whose layouts name both:
    # `ValueError: dictionary update sequence element #0 has length 4`.
    #
    # Empty is the right answer for those: whatever `columns` holds, it is not
    # this block's tuning knobs, and `to_layout` writes the column list from the
    # composition's own `columns` anyway.
    raw = spine.spec.get(section)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not layers_.free:
        # Không còn là cửa từ chối, chỉ là một dòng nói rõ tự do đến từ đâu.
        #
        # Luật cũ: "tự do = chỗ các phôi bất đồng", nên một chứng từ có ĐÚNG
        # MỘT phôi không có tự do nào và mức 3 bỏ qua nó. Đo trên kho này: 36
        # trên 46 chứng từ ở đúng tình cảnh ấy, tức là máy soạn bị khoá ngoài
        # bốn phần năm số loại giấy — và bị khoá đúng ở những loại KHÔNG có
        # phôi thứ hai để nội suy, tức là những loại cần nó nhất.
        #
        # `agent/prompts/anatomy.md` là nguồn tự do thứ hai: kiến thức về giấy
        # tờ ngoài đời thay cho thống kê về kho. Với nó, một phôi vẫn đủ để
        # soạn -- phôi làm bằng chứng "bộ dựng vẽ được cái này", còn hình dáng
        # thì suy từ họ chứng từ. Cửa ải 2..7 không đổi một chữ: tên vẫn phải
        # trong từ vựng đóng, số vẫn phải qua `check_numbers`, trang vẫn phải
        # vẽ ra được và nhãn vẫn phải khớp mực.
        print(f"[compose] {args.document}: {len(refs)} phôi không bất đồng ở "
              f"đâu, nên hình dáng suy từ `agent/prompts/anatomy.md` chứ không "
              f"từ khoảng cách giữa các phôi.")
