...
class Chooser:
    ...
    def __init__(self, rules: dict[str, list[Option]], order: tuple[str, ...],
        ...
        # A HARD ceiling per value, where `pressure` is only a soft one.
        # `weight / (1 + used) ** pressure` never reaches zero, so a document
        # with one drawable layout draws it seven times out of seven pages and
        # no amount of pressure stops it -- measured on `data/llm200_v3`, where
        # 29 layouts went over a "no more than twice" rule and one went eleven
        # times. A cap says the number instead of leaning towards it.
        # Giá trị là MỘT SỐ (trần chung cho mọi giá trị của thuộc tính) hoặc
        # một dict giá trị -> trần. Cần cả hai vì hai trần khác nhau về bản
        # chất: "không bố cục nào quá hai trang" là một số, còn "một chứng từ
        # chỉ được nhiều trang bằng đúng số phôi nó có nhân hai" thì mỗi chứng
        # từ một số khác nhau, đọc từ `blanks.yaml`.
        self.caps: dict[str, int | dict[str, int]] = {}
        for attribute, ceiling in (caps or {}).items():
            self.caps[attribute] = ({str(k): int(v) for k, v in ceiling.items()}
                                    if isinstance(ceiling, dict) else int(ceiling))
    ...
    def take(self, attribute: str, tags: frozenset[str],
             ...
             chosen: Iterable[tuple[str, str]] = (),
             strict_caps: bool = False) -> Option:
        options = self.permitted(attribute, tags, chosen)
        if not options:
            raise Clash(
                f"{attribute}: nothing is drawable under tags {sorted(tags)}")
        cap = self.caps.get(attribute)
        if cap is not None:
            counted = {o.id: self.used[attribute].get(o.id, 0) for o in options}
            ceiling = {o.id: (cap.get(o.id) if isinstance(cap, dict) else cap)
                       for o in options}
            under = [o for o in options
                     if ceiling[o.id] is None or counted[o.id] < ceiling[o.id]]
            if not under and strict_caps:
                # Every legal value is at the ceiling AND the walk still has
                # attempts left: dead-end it so `decide_one` redraws the whole
                # page. The point is the attribute BEFORE this one -- a layout
                # pool is exhausted because of the document that was drawn, and
                # the only way out is a different document. Spreading here
                # would take the overshoot silently; raising lets the walk go
                # and find a document whose phôi are still free.
                raise Clash(
                    f"{attribute}: every legal value is at its ceiling "
                    f"under tags {sorted(tags)}")
            # Last attempts run with `strict_caps` off: the cap is a preference
            # between drawable things, not a rule about what is drawable, and a
            # plan with fewer phôi than pages must still finish. Spread over
            # the least-used so the overshoot lands evenly.
            fewest = min(counted.values())
            options = under or [o for o in options if counted[o.id] == fewest]
    ...
    def at_ceiling(self, attribute: str, option_id: str) -> bool:
        """Has this value already been drawn as often as its cap allows?"""
        cap = self.caps.get(attribute)
        if cap is None:
            return False
        ceiling = cap.get(option_id) if isinstance(cap, dict) else cap
        if ceiling is None:
            return False
        return self.used[attribute].get(option_id, 0) >= int(ceiling)
...
def decide_one(chooser: Chooser, policy, index: int, seed: int,
    ...
    for attempt in range(ATTEMPTS):
        # The last eighth of the attempts drop the ceilings. A cap that could
        # dead-end a page forever would turn "draw fewer repeats" into "fail to
        # draw", which is a worse answer than one repeat too many.
        strict_caps = attempt < ATTEMPTS - max(1, ATTEMPTS // 8)
        # The model's picks are honoured on the FIRST attempt only. A retry
        # exists because the first walk dead-ended, and the proposal is the
        # same every time -- re-applying it walks into the same dead end until
        # the ceilings are dropped, which is how a capped run still came out
        # with eleven layouts over the limit while every page said `by: llm`.
        # After the first attempt the objective picks, and it respects caps.
        proposal = proposal if attempt == 0 else {}
        ...
        try:
            for attribute in chooser.order:
                ...
                # A ceiling the model's own pick may not step over. Without
                # this the cap holds only in coverage mode: `find` answers
                # "is it legal", and on the last run the model picked all 200
                # pages, so every ceiling would have been decoration. Treated
                # exactly like a pick the rules refused -- noted, then drawn by
                # the objective instead, which does respect the ceiling.
                if option is not None and chooser.at_ceiling(attribute, option.id):
                    rejected.append(f"{attribute}={wanted} (đã chạm trần)")
                    option = None
                elif wanted and option is None:
...
def plan(count: int, seed: int, rules: dict[str, list[Option]], policy,
    ...
    chooser = Chooser(rules, order, seed=seed, pressure=pressure,
                      penalty=penalty, ban=ban, caps=caps)
