...
class Variant:
    ...
    # Axis 3, and the one value of it no other source can know. A design that
    # paints the column heads dark and sets `color:#fff` has REVERSED that
    # band -- light type out of a dark fill -- and the ink of every run inside
    # it is `reversed`, not `print`.
    #
    # Declared by the design rather than sniffed out of its CSS. Reading
    # `background:#...` and `color:#fff` back out of a stylesheet is the kind
    # of inference that is right until a design writes the same thing another
    # way; a boolean beside the CSS is right because the author said so.
    #
    # None on a composed dressing: `agent/variants.py`'s own `band` axis has
    # six values and every one of them is a LIGHT fill (`#ececea`, `#e4ecf6`,
    # `#e3f1e6`) under the default dark type, so nothing it composes is ever
    # reversed.
