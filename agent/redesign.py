...
class Design:
    ...
    # Whether this design reverses the item table's column heads: light type
    # out of a dark fill. Axis 3 (`pipeline/record.py::INK_VALUES`) calls that
    # `reversed`, and it is the one ink value only the design itself knows --
    # `kind` cannot say it and the recipe's tags cannot either.
...
def as_variants(designs=None) -> list:
    ...
    return [Variant(id=design.id, level="free", label=design.label,
                    ...
                    reversed_header=design.reversed_header)
