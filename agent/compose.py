...
CORPUS_ROOT = REPO_ROOT / "rulebase" / "corpus"
CORPUS_DIR = CORPUS_ROOT / "vi"          # giữ tên cũ: có nơi khác đọc

# Ngôn ngữ mặc định của một trang khi `content` không nói gì. Tiếng Việt, cùng
# lý do `rulebase/corpus.py::DEFAULT_LANG` chọn thế: kho corpus tiếng Việt là
# kho đầy đủ, tiếng Anh là kho thêm vào sau.
DEFAULT_LANG = "vi"
...
class ComposedPage:
    ...
    document: str
    profile: str
    # Which language this page prints in, from the `content` attribute's own
    # `params.lang` -- the same key `rulebase/content.py::build` reads to pick
    # a corpus. Held per page because the prompt, the character envelope and
    # the diacritic gate all change with it, and a block mixing the two would
    # be asking one prompt to answer for both.
...
def _corpus_text(stem: str, lang: str = DEFAULT_LANG) -> str:
    path = CORPUS_ROOT / lang / f"{stem}.txt"
    return path.read_text(encoding="utf-8") if path.exists() else ""


def _store_name_bounds(profile: str, lang: str = DEFAULT_LANG):
    """The shape and the character envelope a store name is held to.

    Measured off the corpus OF THIS PAGE'S LANGUAGE. Measuring an English name
    against the Vietnamese envelope would refuse every one of them: the
    envelope is the set of characters the committed lines actually use, and a
    Vietnamese one carries `ăâđêôơư` and their tones while an English one does
    not -- so the two are not interchangeable in either direction.
    """
    stem = f"shops_{profile}"
    return (corpus_rules.shape_of(stem),
            corpus_rules.envelope(_corpus_text(stem, lang), column=0))


def _item_name_bounds(profile: str, lang: str = DEFAULT_LANG):
...
def schema_for(profile: str, document: dict[str, Any], block: int,
    ...
    for key in fields["free_text"]:
        if key == "store.name":
            _shape, envelope = _store_name_bounds(profile, lang)
            props[key] = _string_spec(envelope)
            required.append(key)
        elif key == "item_names":
            _shape, envelope = _item_name_bounds(profile, lang)
...
# Trang tiếng Anh có prompt RIÊNG, không phải bản dịch của prompt dưới. Bảo một
# prompt đòi "TIẾNG VIỆT CÓ DẤU ĐẦY ĐỦ" rồi thêm một câu ngoại lệ là cách chắc
# chắn nhất để nhận về tên nửa Việt nửa Anh: điều thứ nhất của nó nói ngược lại.
SYSTEM_EN = """You write real content for a Vietnamese company's
English-language paperwork -- trading names, line-item names.

Rules:
1. ENGLISH ONLY. No Vietnamese words and no Vietnamese diacritics, not even in
   a company name: this page is the English edition, so "Cong ty TNHH Hai Au"
   is "Hai Au Trading Co., Ltd" here.
2. Real, the way a Vietnamese exporter or an international clinic in Ho Chi
   Minh City writes it -- not a random string, and not a translation so literal
   it reads as one.
3. VARY THE BATCH -- do not use one name twice in the same block.

Return only JSON matching the schema, no explanation."""
...
def _user_message(profile: str, document: dict[str, Any], count: int,
                  lang: str = DEFAULT_LANG) -> str:
    if lang == "en":
        return (f"Write content for {count} pages of the same document kind "
                f"(profile={profile}). English only.")
...
def _validate_free_text(value: Any, shape: corpus_rules.Shape,
                        envelope: corpus_rules.Envelope,
                        lang: str = DEFAULT_LANG) -> str:
    ...
    if problem:
        return problem
    # The diacritic gate is what stops a model quietly answering in unaccented
    # Vietnamese, which is the failure this module was written for. It has
    # nothing to say about English: "Hai Au Trading Co., Ltd" carries no
    # diacritic and is exactly right. So the gate runs for `vi`, and the
    # MIRROR of it runs for `en` -- a Vietnamese word smuggled into an English
    # name is the same mistake seen from the other side, and just as invisible.
    if lang == "vi":
        if not has_diacritics(value):
            return "no Vietnamese diacritic"
    elif has_diacritics(value):
        return "Vietnamese diacritic on an English page"
    return ""


def _decide_block(llm, profile: str, document: dict[str, Any],
                  pages: list[ComposedPage], lang: str = DEFAULT_LANG) -> None:
    """Fill `content`/`rejected`/`by` on every page in `pages`, in place."""
    schema = schema_for(profile, document, len(pages), lang)
    if schema is None or llm is None:
        return
    system = SYSTEM_EN if lang == "en" else SYSTEM
    try:
        answer = llm.decide(system, _user_message(profile, document, len(pages), lang),
                            schema)
    except LLMError:
        return
    ...
    if not isinstance(proposals, list) or len(proposals) != len(pages):
        return
    store_shape, store_env = (_store_name_bounds(profile, lang)
                              if "store.name" in PROFILE_FIELDS[profile]["free_text"]
                              else (None, None))
    item_shape, item_env = (_item_name_bounds(profile, lang)
    ...
    for page, proposal in zip(pages, proposals):
        ...
        if "store.name" in proposal and store_shape is not None:
            problem = _validate_free_text(proposal["store.name"], store_shape,
                                          store_env, lang)
            if problem:
        ...
        if isinstance(names, list) and item_shape is not None:
            for index, name in enumerate(names):
                problem = _validate_free_text(name, item_shape, item_env, lang)
...
def decide(decisions: list, rules: dict, *, llm=None, concurrency: int = 1) -> list[ComposedPage]:
    ...
    # `content.build` merges `{**document, **content}`, so the CONTENT value's
    # own `lang` is what decides the corpus -- and therefore what language this
    # page is in. Read from the same place for the same reason: a page whose
    # pixels come out in English and whose prompt asked for Vietnamese is a
    # page the model was set up to fail.
    by_content = {option.id: option.params for option in rules.get("content", [])}
    pages = []
    for decision in decisions:
        ...
        content_params = by_content.get(decision.force.get("content", ""), {})
        lang = str(content_params.get("lang") or params.get("lang") or DEFAULT_LANG)
        pages.append(ComposedPage(seed=decision.seed, document=document_id,
    ...
    for index, page in enumerate(pages):
        head = pages[blocks[-1][0]] if blocks else None
        if head is not None and head.profile == page.profile \
                and head.lang == page.lang and len(blocks[-1]) < 24:
    ...
    def run_block(indices: list[int]) -> None:
        head = pages[indices[0]]
        document = by_document.get(head.document, {})
        block_pages = [pages[i] for i in indices]
        _decide_block(llm, head.profile, document, block_pages, head.lang)
