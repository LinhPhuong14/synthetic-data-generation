        ...
        out.append(Option.from_dict({
            ...
            **constraint,
            "params": {"label": dressing.label, "level": dressing.level,
                       ...
                       # Axis 3: `sheets/variant.py::apply` reads this and
                       # marks the heads it reverses, so the record can say
                       # `ink: reversed` for the runs inside that band.
                       "reversed_header": bool(dressing.reversed_header)},
        }, ATTRIBUTE))
    return out
