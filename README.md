...
│
├── synthgen/                       # BỘ SINH THỨ HAI — dáng soạn mới, không đọc rulebase/layouts/
│   ├── design.py                   # ngữ pháp bố cục: 32 loại chứng từ x 10 trục kiểu dáng
│   ├── content.py                  # nội dung + mọi phép tính tiền của gói này
│   ├── markup.py                   # Doc + Design -> HTML, giữ hợp đồng <span data-kind>
│   ├── paginate.py                 # LUẬT 80%: cắt trang bằng phép đo trong trình duyệt
│   ├── draw.py                     # một trình duyệt cho cả shard: đo, chụp, dựng bản ghi
│   ├── run.py                      # CLI: chọn seed không trùng dáng, chia shard, báo cáo
│   ├── check.py                    # bảy phép kiểm trên một tập đã sinh
│   └── README.md                   # gói này dùng lại gì của kho, và không dùng lại gì
│
...
## 🆕 10b. Bộ sinh thứ hai: `synthgen/`

`rulebase/` + `generators/html/` vẽ **những phôi kho đã có**: mỗi tờ giấy là
một file trong `rulebase/layouts/`, và đa dạng đến từ việc thay mực, thay
giấy, thay bố cục trong số ấy. `synthgen/` trả lời câu hỏi khác: **một dáng
chưa từng có trong kho thì trông thế nào** — nó dựng dáng từ một ngữ pháp
(khổ giấy × lề × cặp phông × mực × kiểu tiêu đề × kiểu bảng × thứ tự khối),
và khử trùng bằng chữ ký dáng chứ không bằng số lượng file.
...
Nó **không thay** đường cũ và không đọc `rulebase/layouts/`. Cái nó dùng lại
là phép ĐO hộp (`generators/html/page.py`), từ vựng nhãn
(`pipeline/record.py`) và kho chữ tiếng Việt (`rulebase/corpus/vi/`) — ba thứ
quyết định nhãn có đúng hay không, và không thứ nào là bố cục.

Điểm mới đáng kể nhất là **luật cắt trang đo được**: một chứng từ chỉ được cắt
làm N tờ khi cả N tờ đều lấp ít nhất 80% khổ giấy. Đường cũ chia đều số dòng
cho các tờ và chỉ gộp lại khi tờ sau rỗng hẳn, nên nó cắt ra những tờ hai lấp
25–36%. Chi tiết ở [`synthgen/README.md`](synthgen/README.md).
