...
def venv(target):
    ...
    # `--no-cache-dir`: torch CPU là bánh xe ~200 MB và ~900 MB sau khi cài.
    # Có cache thì pip giữ CẢ HAI -- bản tải nằm trong `~/.cache/pip`, bản cài
    # nằm trong venv -- nên đỉnh điểm cần gần gấp đôi chỗ, và cache ấy thường
    # nằm trên Ổ KHÁC với venv. Đã gặp thật: "[Errno 28] No space left on
    # device" giữa chừng lần cài đầu tiên.
    pip = [python, "-m", "pip", "install", "-q", "--no-cache-dir"]
    run(pip + ["--upgrade", "pip"])
    run(pip + ["torch", "torchvision",
               "--index-url", "https://download.pytorch.org/whl/cpu"])
    run(pip + list(REQUIREMENTS))
