...
def round_seal(top: str, bottom: str, middle: list[str], *, seed: int,
    ...
    # Lệch nhẹ theo hạt giống: khắc dấu thủ công không bao giờ đặt dòng chữ
    # chính giữa tuyệt đối. Đây là chỗ seed được phép đụng vào, thay cho cỡ dấu
    # — dấu tròn pháp nhân có đường kính do quy định, không do người đóng.
    skew = rng.uniform(-4.0, 4.0)
    _arc_text(canvas, top_text, centre, top_radius, top_font, mid_deg=skew,
    ...
    if bottom:
        ...
        _arc_text(canvas, bottom_text, centre, side * 0.360, bottom_font,
                  mid_deg=180 + rng.uniform(-4.0, 4.0),
...
def oval_seal(top: str, bottom: str, middle: list[str], *, seed: int,
    ...
    skew = rng.uniform(-4.0, 4.0)
    _arc_text_ellipse(canvas, top_text, centre, text_rx, text_ry, top_font, mid_deg=skew,
                      fill=ink, outward=True, spacing=1.02)
...
# Ba màu, và chỉ ba. Mực dấu Việt Nam thực tế chỉ có chừng ấy: đỏ là màu dấu
# pháp nhân, xanh dương là dấu ngân hàng/kế toán, đen-xám là dấu đã qua máy
# photocopy hoặc dấu cao su văn phòng. Xanh lá từng nằm trong danh sách này và
# đã bỏ: nó có thật trên vài con dấu cũ, nhưng hiếm tới mức một bộ dữ liệu
# 200 tờ mà có nó thì tỷ lệ sai hẳn so với đời sống.
INK_COLOURS = [
    (196, 30, 38),    # đỏ -- màu mực dấu phổ biến nhất
    (26, 58, 148),    # xanh dương đậm -- dấu ngân hàng/kế toán hay dùng
    (58, 58, 62),     # đen xám -- dấu cao su văn phòng, và dấu đã qua bản sao
]
