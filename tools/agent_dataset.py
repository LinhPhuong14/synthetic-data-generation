    ...
    * **a layout that fails leaves nothing behind.** The command removes the
    ...
    if not wanted:
        ...
        for document in constraints_mod.load().documents():
            # Every reviewed document with phôi to read, not only the ones whose
            # phôi disagree with each other. That second condition used to be
            # here and it cost four fifths of the rule base: 36 of 46 documents
            # have exactly ONE phôi, so nothing disagrees, so level 3 skipped
            # them -- and it skipped them precisely where there is no second
            # design to interpolate between, which is where composing is worth
            # most. `agent/prompts/anatomy.md` is what replaced that condition:
            # the shape comes from knowing how Vietnamese paperwork is built,
            # and the phôi is evidence of what the builder can draw.
            try:
                compose_layout.references(document)
            except compose_layout.ComposeError:
                continue
            wanted.append(document)
    if not wanted:
        print("[agent] mức 3: chưa chứng từ nào được duyệt trong "
              "agent/constraints.yaml — bỏ qua bước soạn")
        return []
...
def main() -> int:
    ...
    parser.add_argument(
        "--cap", action="append", default=[], metavar="ATTRIBUTE=N",
        help="hard ceiling on how many pages one value of an attribute may "
             "draw, e.g. --cap layout=2. `--pressure` is only a soft pull and "
             "never reaches zero, so a document with one drawable layout keeps "
             "drawing it; this says the number. When every legal value of an "
             "attribute is already at the ceiling the walk still draws -- from "
             "the least-used, so the overshoot spreads instead of piling up.")
    parser.add_argument(
    ...
    caps: dict[str, int] = {}
    for raw in args.cap:
        attribute, _, number = str(raw).partition("=")
        if not number.strip().isdigit() or not attribute.strip():
            raise SystemExit(f"--cap {raw}: viết dạng ATTRIBUTE=N, ví dụ layout=2")
        caps[attribute.strip()] = int(number)
    # Trần của một CHỨNG TỪ suy ra từ trần của bố cục, không viết cứng: một
    # chứng từ có k phôi chỉ chở nổi k×N trang mà không phôi nào quá N lần.
    # Không có bước này thì trần bố cục vô hiệu đúng ở chỗ đau nhất -- chứng từ
    # một phôi mà kế hoạch chia cho bảy trang thì vẽ phôi ấy bảy lần, và trần
    # không có gì để rẽ sang. Đọc từ `rulebase/blanks.yaml`, tức là từ dữ liệu
    # đã có, chứ không phải một bảng số thứ hai phải nhớ cập nhật.
    if "layout" in caps and "document" not in caps:
        from rulebase import blanks as rule_blanks
        # `resolved` chứ không phải danh sách trong `blanks.yaml`: cái quyết
        # định một chứng từ vẽ được bằng bố cục nào là TAG, và danh sách kia là
        # chủ đích của người viết luật. Trần phải tính trên cái vẽ được thật.
        per_document = rule_blanks.resolved(rules)
        derived = {name: caps["layout"] * len(drawable)
                   for name, drawable in per_document.items() if drawable}
        if derived:
            caps["document"] = derived
            print(f"[agent] trần chứng từ suy ra từ blanks.yaml: "
                  f"{min(derived.values())}–{max(derived.values())} trang/chứng từ")
    if caps:
        print("[agent] trần cứng: "
              + ", ".join(f"{a} ≤ {n if not isinstance(n, dict) else 'theo từng giá trị'}"
                          for a, n in sorted(caps.items())))
    ...
    decisions = planner.plan(args.count, args.seed, rules, pol,
                             ...
                             penalty=weights, ban=bans, caps=caps,
