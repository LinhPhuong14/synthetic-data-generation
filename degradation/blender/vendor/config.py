...
# ========== RENDER SETTINGS ==========
# Đọc được từ môi trường, vì đây là bốn con số quyết định TOÀN BỘ chi phí của
# đường warp hình học (đo được: ~98 giây/trang ở mặc định dưới đây) và một lượt
# chạy phải chọn được điểm đánh đổi tốc độ/chất lượng của nó mà không sửa file
# vendor. Mặc định giữ nguyên giá trị cũ, nên không đặt biến nào thì không đổi
# gì. `scene_setup.py` chạy TRONG Blender và import module này, nên biến môi
# trường phải có mặt ở tiến trình `blender` — `render.py` truyền cả os.environ
# xuống subprocess, nên export ở shell là đủ.
import os as _os
...
RENDER_ENGINE = _env("VLM_BLENDER_ENGINE", "CYCLES")  # 'CYCLES', 'BLENDER_EEVEE_NEXT'
...
RENDER_SAMPLES = _env("VLM_BLENDER_SAMPLES", 48, int)
...
USE_DENOISING = _env_bool("VLM_BLENDER_DENOISE", True)  # CYCLES only
RESOLUTION_X = _env("VLM_BLENDER_RES_X", 1536, int)
RESOLUTION_Y = _env("VLM_BLENDER_RES_Y", 2048, int)
RESOLUTION_PERCENTAGE = _env("VLM_BLENDER_RES_PCT", 100, int)  # scale factor applied to the resolution above
FILE_FORMAT = "PNG"
COLOR_DEPTH = "8"  # '8', '16'
